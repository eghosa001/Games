...
func test_main_gameplay_chain() -> void:
    ...
    game.buy_inputs()
    check(game.economy.resources.has("timber") and game.economy.resources.has("iron") and game.economy.resources.has("energy"), "chain buy canonical V1 inputs")
    check(int(game.economy.resources["timber"]["stock"]) >= 12, "chain timber input stock")
    game.produce_goods()
