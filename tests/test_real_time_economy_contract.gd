...
func run() -> void:
    ...
    check(ui_source.contains('"SELL GOODS"') and ui_source.contains("parent.sell_goods()"), "primary UX exposes manual selling")
    check(ui_source.contains('"DELIVER CONTRACT"') and ui_source.contains("parent.deliver_contract()"), "business UX exposes manual contract delivery")
    ...
    check(mobile_source.is_empty(), "obsolete mobile compatibility file is removed")
