...
func run() -> void:
    ...
    var coordinator: Node = get_root().get_node_or_null("RenewUIRegionCoordinator")
    check("coordinator autoload resolves", coordinator != null)
    if coordinator == null:
        game.free()
        quit(1)
        return
    ...
    # ── No screen open — layout-driven visibility ─────────────────────
    ...
    # Desktop-sized viewport: both floaters should compute positions.
    ...
    # ── Mobile-sized viewport: strategy yields ────────────────────────
    ...
    # Tutorial may show above the dock on mobile; just verify no crash.

    # ── Screen open suppresses all floaters ───────────────────────────
    if strat_root != null: strat_root.size = Vector2(1280, 720)
    strategy._layout_responsive()
    tutorial._layout_responsive()
    ...
    # ── Screen close restores floaters ────────────────────────────────
    if smgr != null:
    ...
    # ── Action-dock overlap suppression ───────────────────────────────
    ...
    if action_dock != null:
        ...
        # With desktop size, strategy sits in upper-right; dock is lower.
        # If they happen to overlap, coordinator should hide strategy.
        var overlaps := dock_rect != Rect2() and strat_rect != Rect2() and \
                        dock_rect.intersects(strat_rect)
        if overlaps:
