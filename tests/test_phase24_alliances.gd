...
func run() -> void:
    ...
    check(bool(system.promote_to_director(id, "player", "member_1").get("ok", false)), "Founder can promote a director")
    check(str(system.get_alliance(id).get("members", {}).get("member_1", {}).get("role", "")) == "director", "Director role is stored")
    check(bool(system.demote_to_member(id, "player", "member_1").get("ok", false)), "Founder can demote a director")
    check(str(system.get_alliance(id).get("members", {}).get("member_1", {}).get("role", "")) == "member", "Demotion restores member role")
    ...
    var allocated: Dictionary = system.allocate_treasury_to_railway("player", 500)
    check(bool(allocated.get("ok", false)), "Founder can allocate treasury funds")
    check(int(allocated.get("treasury", -1)) == 500, "Treasury allocation debits alliance treasury")
