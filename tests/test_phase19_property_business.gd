...
func run() -> void:
    ...
    var first_purpose: Dictionary = business.get_business_purpose(0)
    check(not str(first_purpose.get("name", "")).is_empty() and str(first_purpose.get("industry_id", "")) == "furniture", "Warehouse exposes a furniture business purpose")
    var launch_cash_before := int(state.get_value("economy", "cash", 0))
    var launch_cost := int(first_purpose.get("launch_cost", 0))
    ...
    check(state.get_value("businesses", "business_name", "") == first_purpose.get("name", ""), "Business stores property-specific name")
    check(state.get_value("businesses", "business_purpose", "") == first_purpose.get("id", ""), "Business purpose is canonical")
    ...
    check(state.get_value("economy", "cash", 0) == launch_cash_before - launch_cost, "Business launch consumes working capital")
