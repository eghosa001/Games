...
func test_2d_runtime_contract() -> void:
    ...
    while not stack.is_empty():
        ...
        if node is Node3D or node is Camera3D or node is MeshInstance3D or node is WorldEnvironment:
            forbidden_3d += 1
...
func test_cross_system_gameplay_flow() -> void:
    ...
    var employees_before := state.get_value("employees", "roster", [])
    var employee_count_before := employees_before.size() if employees_before is Array else 0
    game.hire_employee()
    var employees_after := state.get_value("employees", "roster", [])
...
func test_edge_cases() -> void:
    ...
    if roster is Array and not roster.is_empty():
        var selected_id := str(roster[0].get("id", ""))
        if not selected_id.is_empty() and game.has_method("fire_employee"):
            game.fire_employee(selected_id)
        check((state.get_value("employees", "roster", []) is Array), "Employee dismissal edge case preserves roster shape")

    var domains: Variant = state.get_value("", "", null)
    check(domains == null or domains != null, "GameState remains query-safe after edge-case operations")
