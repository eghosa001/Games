...
func run() -> void:
    ...
    check(chain.has_method("sell_product"), "Supply chain exposes generic product sales")
    ...
    var generic_sale = chain.sell_product("furniture", 2, 220)
    check(bool(generic_sale.get("ok", false)) and int(generic_sale.get("sold", 0)) == 2, "Generic product sale succeeds")
    check(chain.stock("furniture") == 1.0, "Generic product sale reduces warehouse stock")
    var furniture_alias_sale = chain.sell_furniture(1, 220)
    check(bool(furniture_alias_sale.get("ok", false)) and int(furniture_alias_sale.get("sold", 0)) == 1, "Furniture sale alias uses generic product path")
    check(chain.stock("furniture") == 0.0, "Furniture alias sale leaves warehouse at zero")
