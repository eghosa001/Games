...
func _ready() -> void:
    ...
    var restored = EmployeeSystem.new(); add_child(restored); restored.restore_state(snapshot)
    ...
    var before_skill = int(hired["skills"]["production"])
    var train = restored.train_employee(str(hired["id"]), 2)
    if not train["ok"] or int(hired["skills"]["production"]) <= before_skill: failures.append("Training should improve employee specialization skill")
    ...
    var legacy = EmployeeSystem.new(); add_child(legacy); legacy.employees.clear(); legacy.migrate_legacy_count(5, 1)
