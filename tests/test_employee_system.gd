...
func run() -> void:
    ...
    if not train["ok"] or int(trained.get("skills", {}).get("production", 0)) <= before_skill: failed += 1;failures.append("Training should improve employee specialization skill")
    ...
    if int(restored.get_employee(EmployeeSystem.JAMES_ID)["level"]) < 2: failed += 1;failures.append("Promotion should increase employee level")
