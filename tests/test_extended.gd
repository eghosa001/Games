...
func _reset_ledger(finance: Node, state: Node, cash: int) -> void:
    finance.financing = {}
    finance.debt = 0
    ...
    finance.equity_contributed = float(max(0, cash))
    finance.cash = max(0, cash)
...
func run() -> void:
    ...
    if tech_system != null:
        ...
        var direct_entry = state.get_value("technology", "technology", {}).get("efficient_production", null)
        check(direct_entry is Dictionary and direct_entry.get("researched", false) != true, "Pending technology research persists in GameState")
        ...
        var blocked_technology: Dictionary = state.get_value("technology", "technology", {})
        check(not blocked_technology.has("efficient_production"), "Research is blocked before business launch")
        ...
        check(research_days == 2, "Technology reports its calendar research duration")
        check(int(tech_system.get_last_research_days()) == 0, "Research command never reports hidden elapsed days")
        check(int(state.get_value("player", "day", 0)) == 10, "Research command does not skip canonical simulation days")
        for _i in range(research_days):
            tech_system.advance_calendar_day()
        check(tech_system.is_unlocked("efficient_production"), "Calendar rollover completes pending technology research")
        check(int(state.get_value("economy", "cash", 0)) == int(finance.get("cash")), "Research calendar flow keeps finance and GameState synchronized")
    ...
    _reset_ledger(finance, state, 1000)
    ...
    _reset_ledger(finance, state, 10000)
    ...
    finance_fixture.equity_contributed = 35000.0
    finance_fixture.retained_earnings = 0.0
    ...
    check(bool(finance_fixture.validate_invariants().get("ok", false)), "Finance fixture starts with valid debt invariants")
