...
func run() -> void:
    ...
    if s != null:
        ...
        for n in ["RegionController","Corporate","SupplyChainController","WorldMissions","MarketDirector","MobileUI","BranchController"]: check(g.get_node_or_null(n) != null, "Controller " + n)
        check(g.economy != null and g.production != null and g.districts != null and g.expansion != null, "Core RefCounted systems")
        check(g.rivals.ai_status(0).has("personality"), "Persistent competitor AI profile")
        ...
        var bc = g.get_node("BranchController")
        for n in ["select_branch","launch_selected","stock_selected","hire_selected","upgrade_selected","price_selected"]: check(bc.has_method(n), "BranchController " + n)
        bc.select_branch(1); check(bc.branches.selected == 1, "BranchController select")
    ...
    var ai = load("res://scripts/competitors.gd").new(); var ai_before = ai.ai_status(0); ai.strategic_ai_update(1, {"cash":25000,"reputation":50,"player_price":110,"selected_district":0,"acquisition_count":1}); var ai_after = ai.ai_status(0); check(ai_after["last_strategy"] != "", "Competitor AI strategy"); check(ai_after["last_action"] != "", "Competitor AI execution"); check(ai_after["memory_size"] > ai_before["memory_size"], "Competitor AI memory")
