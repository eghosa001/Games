...
func run() -> void:
    ...
    var before_cash := int(ai.rivals[0]["cash"])
    ...
    check(int(ai.rivals[0]["cash"]) != before_cash or ai.rivals[0]["last_action"] == "optimize_operations", "AI changes corporate state or records optimization")
    var snapshot = ai.rivals.duplicate(true)
    var restored = Script.new(); restored.rivals = snapshot; restored._normalize()
