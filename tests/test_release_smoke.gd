...
func run() -> void:
	...
	for path in ["res://data/properties.json","res://data/industries.json","res://data/resources.json","res://data/employees.json","res://data/competitors.json","res://data/technologies.json"]:
	...
	if scene != null:
		...
		for path in ["World/WorldView","World/RegionController","World/PropertyVisual","Systems/GameBalance","Systems/Progression","Systems/FinanceSystem","UI/MainHUD","UI/TechnologyPanel","UI/HistoryPanel"]:
		...
		check(finance_scene_node != null and finance_scene_node.get_script() != null and str(finance_scene_node.get_script().resource_path).ends_with("finance_scene_bridge.gd"), "Scene FinanceSystem uses finance_scene_bridge.gd")
