...
func run()->void:
    ...
    check(tech.research("smart_factory")==false,"Tier 3 remains locked without its prerequisite")
    tech.add_daily_research_points(3)
    check(int(state.get_value("technology","research_points",0))==6,"Daily research points are awarded")
