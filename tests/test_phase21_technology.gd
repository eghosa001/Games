...
func run()->void:
    ...
    var start_cash=int(finance.cash);var start_points=int(state.get_value("technology","research_points",20))
    check(tech.research("efficient_production"),"Efficient Production research starts")
    ...
    check(int(tech.get_last_research_days())==0,"Research never hides day simulation")
    check(int(tech.get_last_research_duration_days())==2,"Research exposes its two-day duration")
    check(not tech.is_unlocked("efficient_production"),"Technology remains pending before calendar rollover")
    tech.advance_calendar_day()
    check(not tech.is_unlocked("efficient_production"),"Technology remains pending after one day")
    tech.advance_calendar_day()
    check(tech.is_unlocked("efficient_production"),"Technology unlocks after required calendar days")
    check(tech.production_multiplier()>1.0,"Completed technology changes production multiplier")
    tech.add_daily_research_points(10)
    check(tech.research("automation"),"Automation research starts after prerequisite")
    for _i in range(3): tech.advance_calendar_day()
    check(tech.is_unlocked("automation"),"Automation completes after its calendar duration")
    ...
    var before_points:=int(state.get_value("technology","research_points",0))
    tech.add_daily_research_points(3)
    check(int(state.get_value("technology","research_points",0))==before_points+3,"Daily research points are awarded")
