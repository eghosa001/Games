...
func run()->void:
    ...
    var efficient_days:=int(tech.get_last_research_duration_days())
    check(efficient_days==2,"Research records real-calendar duration")
    check(not tech.is_unlocked("efficient_production"),"Technology stays pending until calendar completion")
    check(is_equal_approx(tech.production_multiplier(),1.0),"Pending technology has no production effect")
    for _day in range(efficient_days):
    ...
    check(tech.research("automation"),"Automation starts after prerequisite")
    var automation_days:=int(tech.get_last_research_duration_days())
    for _day in range(automation_days):
        tech.advance_calendar_day()
    check(tech.is_unlocked("automation"),"Automation completes after calendar progress")
    ...
    var points_before_daily:=int(state.get_value("technology","research_points",0))
    tech.add_daily_research_points(3)
    check(int(state.get_value("technology","research_points",0))>points_before_daily,"Daily research points are awarded")
