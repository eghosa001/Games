...
func run() -> void:
    ...
    check(snapshot.get("state_version", 0) == 5, "GameState writes current version")
    ...
    check(state.get_value("employees", "employees", 0) == 4, "Employees domain owns employee count")
    ...
    state.set_value("economy", "cash", 18000)
    check(state.get_value("economy", "cash", 0) == 18000, "Canonical economy write works")
    state.set_value("player", "day", 9)
    check(state.get_value("player", "day", 0) == 9, "Canonical player write works")

    var before := state.get_domain("economy")
