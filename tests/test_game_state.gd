...
func run() -> void:
    ...
    var state = State.new()
    state.name = "TestGameStateA"
    root.add_child(state)
    await process_frame
    state.set_value("economy", "cash", 25000)
    state.set_value("player", "day", 7)
    state.set_value("player", "reputation", 12)
    state.set_value("properties", "owned", true)
    state.set_value("businesses", "business_open", true)
    state.set_value("employees", "roster", [{"id":"emp_001","status":"active"}])
    var snapshot: Dictionary = state.capture()
    check(int(snapshot.get("schema_version", 0)) == 8, "GameState writes schema version 8")
    check(snapshot.has("domains"), "GameState contains canonical domain map")
    check(not snapshot.has("legacy"), "New GameState saves do not contain legacy payload")
    ...
    check(state.get_value("player","day",0) == 7, "Player owns day")
    check(state.get_value("player","reputation",0) == 12, "Player owns reputation")
    check(state.get_value("properties","owned",false), "Properties owns property state")
    ...
    restored.name = "TestGameStateB"
    root.add_child(restored)
    await process_frame
    check(bool(restored.restore(snapshot)), "Canonical schema restores successfully")
    check(restored.get_value("economy","cash",0) == 25000, "Restored economy cash")
    check(restored.get_value("player","day",0) == 7, "Restored player day")
    check(restored.get_value("employees","roster",[]).size() == 1, "Restored employee roster")
