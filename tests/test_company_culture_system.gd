...
func run() -> void:
    ...
    for dimension in CultureSystem.DIMENSIONS:
        if not profile.has(dimension):
            failed += 1;failures.append("Culture profile missing dimension: %s" % dimension)
        elif int(profile[dimension]) < 0 or int(profile[dimension]) > 100:
            failed += 1;failures.append("Culture dimension out of range: %s" % dimension)
    ...
    if not bool(changed.get("ok", false)) or system.get_dimension("innovation") != min(before + 20, 100):
        failed += 1;failures.append("Culture dimension adjustment should be bounded and persistent")
    ...
    if not bool(clamped.get("ok", false)) or system.get_dimension("quality") != 100:
        failed += 1;failures.append("Culture values must clamp to 100")
    ...
    for key in ["recruitment_multiplier", "retention_multiplier", "productivity_multiplier", "research_multiplier", "quality_multiplier", "expansion_multiplier", "reputation_multiplier"]:
        if not effects.has(key) or float(effects[key]) <= 0.0:
            failed += 1;failures.append("Culture effects missing or invalid: %s" % key)
    ...
    if bool(daily_second.get("changed", true)):
        failed += 1;failures.append("Company culture daily update must be idempotent for the same day")
    if after_first.get("culture", {}) != after_second.get("culture", {}):
        failed += 1;failures.append("Repeated same-day culture updates must not apply modifiers twice")
    if int(after_first.get("last_day", 0)) != 10:
        failed += 1;failures.append("Culture daily update must record the processed day")
    ...
    if restored.get_dimension("innovation") != system.get_dimension("innovation"):
        failed += 1;failures.append("Culture must survive capture/restore")
    if restored.get_dimension("quality") != 100:
        failed += 1;failures.append("Restored culture must retain bounded values")
    if int(restored.capture_state().get("last_day", 0)) != 10:
        failed += 1;failures.append("Processed culture day must survive capture/restore")

    var invalid := restored.set_dimension("not_a_dimension", 50)
    if bool(invalid.get("ok", true)):
        failed += 1;failures.append("Unknown culture dimensions must be rejected")
