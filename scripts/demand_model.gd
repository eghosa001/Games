...
func calculate(product: String, player_price: float, competitor_price: float, reputation: int, quality: int, marketing_level: int, contract_bonus: int, employee_productivity: float, district_multiplier: float, district_pressure: float, alliance_sales: float, deal_sales: float) -> Dictionary:
    ...
    var effective_district := clampf(district_multiplier * (1.0 - district_pressure), 0.60, 1.40)
    ...
    var employee_modifier := clampf(employee_productivity, 0.45, 1.55)
    var relationship_modifier := clampf(1.0 + alliance_sales * 0.45 + deal_sales * 0.50, 0.50, 1.90)
    ...
    # Tests may instantiate demand models outside the active SceneTree. Optional global systems are therefore queried only when available.
    if is_inside_tree():
        var tech: Node = get_node_or_null("/root/RenewTechnologySystem")
        if tech != null: market_multiplier = float(tech.market_demand_multiplier())
    ...
    var demand: int = clampi(int(round(demand_float)), int(config.get("min_demand", 0)), int(config.get("max_demand", 100)))
    ...
    for segment in segment_details.keys():
        ...
        detail["final_demand"] = int(round(float(detail.get("demand", 0)) * employee_modifier * relationship_modifier * market_multiplier * event_multiplier))
    ...
    var price_modifier := clampf(pow(maxf(0.01, relative_price), price_strength), 0.20, 1.80)
