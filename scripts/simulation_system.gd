...
func _settle_sales(args: Dictionary) -> Dictionary: return _finance().settle_sales(int(args.get("sales", 0)), int(args.get("wages", 0)), int(args.get("overhead", 0)), int(args.get("contract_income", 0)))
func advance_day(state: Dictionary, context: Dictionary) -> Dictionary:
    ...
    state["cash"] = int(state.get("cash", 0)) + profit; state["finished_goods"] = max(0, finished_goods); state["last_sales"] = sales + contract_income; state["last_customer_demand"] = customer_demand; state["last_units_sold"] = units_sold; state["last_price_modifier"] = float(demand_modifiers.get("price", 1.0)); state["last_demand_modifiers"] = demand_modifiers.duplicate(true); state["last_demand_raw"] = float(demand_result.get("raw_demand", customer_demand)); state["last_relative_price"] = float(demand_result.get("relative_price", 1.0)); state["last_profit"] = profit; state["total_profit"] = int(state.get("total_profit", 0)) + profit
    ...
    var district_name: String = str(districts.current()["name"]); _append_log(state, "DAY %d: %d sold | demand %d | price $%s vs rival $%s | sales $%s | profit $%s | production cost $%s | district %s." % [state["day"], units_sold, customer_demand, _money(player_price), _money(rival_price), _money(sales), _money(profit), _money(production_operating_cost), district_name]); state["message"] = "Day %d closed. %d/%d demand sold at $%s; profit $%s; production cost $%s; empire profit $%s." % [state["day"], units_sold, customer_demand, _money(player_price), _money(profit), _money(production_operating_cost), _money(int(empire["profit"]))]
    var production_system = _production(); if production_system != null: production_system.advance_day()
...
func restore_state(snapshot: Dictionary) -> void:
    ...
    command_count = int(snapshot.get("command_count", 0)); last_command = str(snapshot.get("last_command", "")); last_result = snapshot.get("last_result", {}).duplicate(true); var finance = _finance()
