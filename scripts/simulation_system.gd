...
func advance_day(state: Dictionary, context: Dictionary) -> Dictionary:
    ...
    var rivals = context.get("rivals"); var economy = context.get("economy"); var events = context.get("events"); var expansion = context.get("expansion"); var districts = context.get("districts")
    ...
    var price_factor := clamp(float(rival_price - int(state.get("player_price", 110))) / 50.0, -0.55, 0.75); var district_mult := districts.business_multiplier("Consumer Goods")
    var reputation := int(state.get("reputation", 0)); var marketing_level := int(state.get("marketing_level", 0))
    ...
    var employees := int(state.get("employees", 3)); var capacity_level := int(state.get("capacity_level", 1)); var wages := employees * 180; var overhead := 650 + capacity_level * 100
    ...
    var empire = expansion.operate_day(); state["cash"] += int(empire["profit"])
    ...
    var district_name := str(districts.current()["name"]); _append_log(state, "DAY %d: %d sold | sales $%s | core profit $%s | district %s." % [state["day"], units, _money(sales), _money(profit), district_name])
...
func end_day(args: Dictionary = {}) -> Dictionary:
    ...
    var debt_result := finance.settle_debt_day(); var economy = _economy(); if economy != null: economy.end_market_day(); last_result = debt_result; return debt_result
