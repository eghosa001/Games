...
func advance_day(state: Dictionary, context: Dictionary) -> Dictionary:
    ...
    # START DAY -> Employee daily update.
    ...
    # Resource market update happens before production so today's production sees current prices/availability.
    economy.end_market_day()

    # Production is now part of the daily simulation rather than a separate prerequisite.
    ...
    # Wages are settled after sales and contracts, using authoritative employee salaries.
    ...
    ai_player_state["selected_rival"] = selected_rival
    ai_player_state["selected_district"] = selected_district
    ...
    # History + news: the chronological log is the current history/news feed.
    var district_name: String = str(districts.current()["name"])
    _append_log(state, "DAY %d: %d sold | sales $%s | profit $%s | district %s." % [state["day"], units, _money(sales), _money(profit), district_name])
    state["message"] = "Day %d closed. %d sold; profit $%s; empire profit $%s." % [state["day"], units, _money(profit), _money(int(empire["profit"]))]

    return {"ok": true, "message": state["message"], "state": state.duplicate(true), "contract": contract_result, "wages": wages, "employee_update": employee_result}
...
func _execute_contract(state: Dictionary, available_after_sales: int) -> Dictionary:
    ...
    if bool(result.get("finished", false)):
        ...
        state["contract_days"] = 0; state["contract_bonus"] = 0
