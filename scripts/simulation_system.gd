...
func advance_day(state: Dictionary, context: Dictionary) -> Dictionary:
    ...
    var rivals=context.get("rivals"); var economy=context.get("economy"); var events=context.get("events"); var expansion=context.get("expansion"); var districts=context.get("districts"); var employee_system=context.get("employee_system")
    if rivals==null or economy==null or events==null or expansion==null or districts==null: return {"ok":false,"message":"Simulation domain dependencies are unavailable."}
    var selected_rival:=int(state.get("selected_rival",0)); var selected_district:=int(state.get("selected_district",0))
    var rival_price:int=int(rivals.rivals[0]["price"]); var alliance:Dictionary=rivals.alliance_bonus(selected_rival); var deal:Dictionary=rivals.deal_bonus(selected_rival); var pressure:float=float(rivals.district_pressure(selected_district))
    var price_factor:float=clamp(float(rival_price-int(state.get("player_price",110)))/50.0,-0.55,0.75); var district_mult:float=float(districts.business_multiplier("Consumer Goods")); var reputation:=int(state.get("reputation",0)); var marketing_level:=int(state.get("marketing_level",0))
    ...
    var units:=min(demand,finished_goods); var player_price:=int(state.get("player_price",110)); var sales:int=units*player_price
    var wages:=int(state.get("wages",0)); var morale_multiplier:=1.0
    if employee_system!=null:
        wages=employee_system.get_daily_wage_total()
        morale_multiplier=employee_system.get_morale_multiplier()
    else: wages=int(state.get("employees",0))*180
    var capacity_level:=int(state.get("capacity_level",1)); var overhead:=650+capacity_level*100
    var contract_income:=0; var contract_penalty:=0; var contract_result:Dictionary=_execute_contract(state,finished_goods-units)
    ...
    profit=int(round(float(profit)*morale_multiplier))
    state["cash"]=int(state.get("cash",0))+profit; finished_goods-=units; state["finished_goods"]=max(0,finished_goods)
    state["last_sales"]=sales+contract_income; state["last_profit"]=profit; state["total_profit"]=int(state.get("total_profit",0))+profit
    ...
    state["day"]=int(state.get("day",1))+1; economy.end_market_day()
    for news in rivals.daily_update(int(state["day"])): _append_log(state,"RIVAL: "+news)
    var ai_player_state:Dictionary=state.duplicate(true); ai_player_state["selected_rival"]=selected_rival; ai_player_state["selected_district"]=selected_district
    ...
    if int(state["day"])%2==0:
        var event:Dictionary=events.roll(); state["cash"]+=int(event["cash"]); state["reputation"]+=int(event["rep"]); _append_log(state,"EVENT: %s — %s"%[event["title"],event["text"]])
    var empire:Dictionary=expansion.operate_day(); state["cash"]+=int(empire["profit"])
    ...
    expansion.unlock_from_reputation(int(state["reputation"])); districts.update_unlocks(int(state["reputation"]))
    var district_name:String=str(districts.current()["name"]); _append_log(state,"DAY %d: %d sold | sales $%s | core profit $%s | district %s."%[state["day"],units,_money(sales),_money(profit),district_name])
    state["message"]="Day %d closed. %d sold; core profit $%s; empire profit $%s."%[state["day"],units,_money(profit),_money(int(empire["profit"]))]
    return {"ok":true,"message":state["message"],"state":state.duplicate(true),"contract":contract_result,"wages":wages,"morale_multiplier":morale_multiplier}

func _execute_contract(state:Dictionary,available_after_sales:int)->Dictionary:
    var contracts=_contracts(); if contracts==null:return {}
    ...
    if bool(result.get("finished",false)):
        ...
        state["reputation"]=max(0,int(state.get("reputation",0))+int(impact.get(rep_key,0))); state["contract_days"]=0; state["contract_bonus"]=0; _append_log(state,"CONTRACT %s: %s."%[final_contract["id"],status.to_upper()])
...
func _append_log(state:Dictionary,text:String)->void:
    ...
    var logs=state.get("log_lines",[]); if logs is Array:logs.append(text); if logs.size()>100:logs.pop_front(); state["log_lines"]=logs
...
func capture_state()->Dictionary:
    ...
    var finance=_finance(); var production=_production(); if finance!=null:result["finance"]=finance.capture_state(); if production!=null:result["production"]=production.capture_state(); return result
