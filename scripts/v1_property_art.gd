...
func _draw() -> void:
    ...
    var stage_index := _stage_index(property, owned)
...
func _draw_environment(o: Vector2, stage: int) -> void:
    ...
    for x in range(440, 881, 40):
        draw_line(Vector2(x, 390), Vector2(x, 600), GRID, 1.0)
    for y in range(410, 601, 40):
        draw_line(Vector2(420, y), Vector2(880, y), GRID, 1.0)
    ...
    if stage == 5:
        draw_circle(Vector2(o.x + 175, o.y - 15), 24, Color(0.90, 0.78, 0.42, 0.08))
...
func _draw_property(o: Vector2, property: Dictionary, stage: int) -> void:
    ...
    var wall := STAGE_COLORS[stage]
    var trim := Color("2b3941").lerp(Color("e4e8e5"), paint)
    var roof := Color("27333a").lerp(Color("52636c"), repair)
    var body := Rect2(o.x - 150, o.y - 105, 300, 210)

    # Ground shadow gives the building a deliberate, premium silhouette.
    draw_ellipse(Vector2(o.x, o.y + 112), Vector2(176, 18), Color(0, 0, 0, 0.34))

    if type == "Workshop":
        _draw_workshop(o, body, wall, roof, trim, stage, repair, paint, furnish)
    elif type == "Commercial Building":
        _draw_commercial(o, body, wall, roof, trim, stage, repair, paint, furnish)
    else:
        _draw_warehouse(o, body, wall, roof, trim, stage, repair, paint, furnish)
...
func _draw_warehouse(o: Vector2, body: Rect2, wall: Color, roof: Color, trim: Color, stage: int, repair: float, paint: float, furnish: float) -> void:
    ...
    draw_rect(Rect2(o.x - 160, o.y - 105, 320, 10), trim, true)
    draw_rect(Rect2(o.x - 160, o.y + 76, 320, 14), trim, true)
    for i in range(5):
        var wx := o.x - 120.0 + i * 60.0
        _draw_window(Vector2(wx, o.y - 56), 28, 38, stage >= 4, furnish)
    ...
    if stage >= 4:
        draw_rect(Rect2(o.x - 28, o.y + 25, 56, 8), ACCENT, true)
    if stage == 5:
...
func _draw_workshop(o: Vector2, body: Rect2, wall: Color, roof: Color, trim: Color, stage: int, repair: float, paint: float, furnish: float) -> void:
    ...
    draw_rect(Rect2(o.x + 92, o.y - 138, 25, 33), roof.darkened(0.15), true)
    draw_rect(Rect2(o.x + 97, o.y - 153, 15, 18), roof.darkened(0.25), true)
    draw_rect(Rect2(o.x - 154, o.y - 100, 308, 9), trim, true)
    for i in range(3):
        _draw_window(Vector2(o.x - 105 + i * 105, o.y - 55), 38, 45, stage >= 4, furnish)
    draw_rect(Rect2(o.x - 62, o.y - 8, 124, 113), Color("26343b"), true)
    draw_rect(Rect2(o.x - 52, o.y + 3, 104, 92), Color("354850").lerp(Color("536a72"), furnish), true)
    if stage >= 3:
        draw_line(Vector2(o.x - 48, o.y + 22), Vector2(o.x + 48, o.y + 22), trim, 5)
    if stage == 5:
        draw_circle(Vector2(o.x + 130, o.y + 65), 8, ACCENT)
...
func _draw_commercial(o: Vector2, body: Rect2, wall: Color, roof: Color, trim: Color, stage: int, repair: float, paint: float, furnish: float) -> void:
    draw_rect(body, wall, true)
    draw_rect(Rect2(o.x - 162, o.y - 108, 324, 16), roof, true)
    draw_rect(Rect2(o.x - 162, o.y + 38, 324, 13), trim, true)
    draw_rect(Rect2(o.x - 162, o.y + 72, 324, 33), Color("25343b"), true)
    for i in range(4):
        var wx := o.x - 118.0 + i * 78.0
        _draw_window(Vector2(wx, o.y - 48), 54, 52, stage >= 4, furnish)
    draw_rect(Rect2(o.x - 36, o.y + 52, 72, 53), Color("1e2a30"), true)
    if stage >= 3:
        draw_rect(Rect2(o.x - 122, o.y + 10, 244, 18), Color("324750").lerp(Color("556e76"), furnish), true)
    if stage == 5:
...
func _draw_window(p: Vector2, w: float, h: float, lit: bool, furnish: float) -> void:
    var glass := Color("1b2a31")
    if lit:
        glass = Color("b7cbd0").lerp(Color("f0d37c"), furnish * 0.75)
    draw_rect(Rect2(p.x - w / 2, p.y - h / 2, w, h), INK, true)
    draw_rect(Rect2(p.x - w / 2 + 4, p.y - h / 2 + 4, w - 8, h - 8), glass, true)
    draw_line(Vector2(p.x, p.y - h / 2 + 4), Vector2(p.x, p.y + h / 2 - 4), Color(0.3, 0.4, 0.43, 0.8), 2)
    draw_line(Vector2(p.x - w / 2 + 4, p.y), Vector2(p.x + w / 2 - 4, p.y), Color(0.3, 0.4, 0.43, 0.8), 2)

func _draw_damage(o: Vector2, body: Rect2, dirt: float, damage: float) -> void:
    if dirt > 0.02:
        for i in range(int(lerpf(1, 12, dirt))):
            var x := body.position.x + 16 + fmod(i * 47.0, body.size.x - 32)
            var y := body.position.y + 18 + fmod(i * 29.0, body.size.y - 36)
            draw_circle(Vector2(x, y), 2.0 + dirt * 2.0, Color(0.08, 0.11, 0.12, 0.22 * dirt))
    if damage > 0.02:
        for i in range(int(lerpf(1, 7, damage))):
            var x := body.position.x + 20 + fmod(i * 61.0, body.size.x - 40)
            var y := body.position.y + 20 + fmod(i * 37.0, body.size.y - 40)
            var c := Color(0.08, 0.10, 0.11, 0.8 * damage)
            draw_line(Vector2(x, y), Vector2(x + 15, y + 10), c, 2.2)
            draw_line(Vector2(x + 10, y + 7), Vector2(x + 5, y + 17), c, 1.6)
        if damage > 0.5:
            for i in range(3):
                draw_rect(Rect2(body.position.x + 20 + i * 105, body.position.y + 145, 44, 5), Color(0.1, 0.12, 0.12, 0.5), true)

func _draw_stage_accents(o: Vector2, body: Rect2, stage: int, furnish: float) -> void:
    if stage >= 1:
        for p in [Vector2(o.x - 178, o.y + 116), Vector2(o.x + 170, o.y + 118)]:
            draw_circle(p, 8, Color("344b3d"))
    if stage >= 3:
        draw_rect(Rect2(o.x - 145, o.y + 108, 290, 6), Color("65767b"), true)
    if stage >= 4:
        draw_rect(Rect2(o.x - 130, o.y + 117, 70, 7), POSITIVE.darkened(0.2), true)
        draw_rect(Rect2(o.x + 60, o.y + 117, 70, 7), POSITIVE.darkened(0.2), true)
    if stage == 5:
        draw_circle(Vector2(o.x - 158, o.y + 112), 5, POSITIVE)
        draw_circle(Vector2(o.x + 158, o.y + 112), 5, POSITIVE)
        draw_rect(Rect2(o.x - 98, o.y + 132, 196, 4), POSITIVE, true)

func _draw_label(o: Vector2, property: Dictionary, stage: int) -> void:
    var panel := Rect2(o.x - 205, o.y + 170, 410, 82)
    draw_rect(panel, Color(0.045, 0.07, 0.09, 0.96), true)
    draw_rect(Rect2(panel.position, Vector2(panel.size.x, 3)), ACCENT if stage == 5 else Color("718793"), true)
    ...
    for i in range(6):
        var c := POSITIVE if i <= stage else Color("26343b")
        draw_circle(Vector2(panel.position.x + 290 + i * 16, panel.position.y + 60), 4.5, c)

func _draw_tree(p: Vector2, scale: float) -> void:
    draw_rect(Rect2(p.x - 3 * scale, p.y, 6 * scale, 22 * scale), Color("4c4435"), true)
    draw_circle(p + Vector2(-9, -4) * scale, 13 * scale, Color("365443"))
    draw_circle(p + Vector2(7, -8) * scale, 15 * scale, Color("3f654c"))
    draw_circle(p + Vector2(0, -18) * scale, 12 * scale, Color("4c7455"))
