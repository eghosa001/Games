...
func _build() -> void:
    ...
    amount_spin.max_value = 500
    amount_spin.value = 10
    amount_spin.custom_minimum_size = Vector2(105, 46)
    row.add_child(amount_spin)
    transport_spin = SpinBox.new()
    transport_spin.min_value = 1
    transport_spin.max_value = 10
    transport_spin.value = 1
    transport_spin.custom_minimum_size = Vector2(105, 46)
    row.add_child(transport_spin)
    procure_button = _button("PROCURE", 46)
    procure_button.size_flags_horizontal = Control.SIZE_EXPAND_FILL
    procure_button.pressed.connect(_procure)
    row.add_child(procure_button)
    content.add_child(_section("PROCESSING"))
    process_button = _button("PROCESS IRON → METAL", 46)
    process_button.pressed.connect(_process_metal)
...
func _refresh(force: bool = false) -> void:
    ...
    if supply == null:
        status.text = "SUPPLY CHAIN OFFLINE"
        procure_button.disabled = true
        process_button.disabled = true
        return
    procure_button.disabled = false
    ...
    var signature := "%d:%d:%s:%s" % [int(total), int(supply.total_freight_cost), str(supply.competitor_pressure), operation_label.text]
...
func _populate_resources(_warehouse: Dictionary) -> void:
    ...
    for id in ids:
        resource_select.add_item(id.replace("_", " ").capitalize())
        resource_select.set_item_metadata(resource_select.item_count - 1, id)
...
func _procure() -> void:
    ...
    var cash := _cash()
    var result: Dictionary = supply.procure(id, amount, cash, int(transport_spin.value))
    operation_label.text = _dict_text(result)
    _refresh(true)

func _process_metal() -> void:
    if supply == null: return
    var result: Dictionary = supply.process_iron_to_metal(int(amount_spin.value))
    operation_label.text = _dict_text(result)
    _refresh(true)
...
func _layout() -> void:
    ...
    var w := minf(size.x - 20.0, 860.0)
    var h := minf(size.y - 24.0, 720.0)
...
func _button(text: String, height: int) -> Button:
    ...
    b.add_theme_stylebox_override("normal", _box(Color("102a31"), Color("31565d"), 9))
    b.add_theme_stylebox_override("hover", _box(Color("18363a"), Color("d8b76d"), 9))
    b.add_theme_stylebox_override("pressed", _box(Color("18363a"), Color("d8b76d"), 9))
    return b
