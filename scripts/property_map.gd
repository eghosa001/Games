...
func _signature() -> String:
    ...
    var parts: Array = [str(state.get_value("player", "day", 1)), str(state.get_value("properties", "selected_property", 0)), str(state.get_value("properties", "owned", false)), str(state.get_value("businesses", "business_open", false))]
    if catalog is Array:
        for entry in catalog:
            if entry is Dictionary:
                parts.append("%s:%d:%d:%d:%d:%s" % [str(entry.get("id", "")), int(entry.get("cleaning", 0)), int(entry.get("repair", 0)), int(entry.get("painting", 0)), int(entry.get("furnishing", 0)), str(entry.get("lease_until", 0))])
    return "|".join(parts)
