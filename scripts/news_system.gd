...
func _ready()->void:
    ...
    call_deferred("_connect_history")
func _process(_delta:float)->void:_watch_day()
func _initialize_from_game()->void:
    var state=get_node_or_null("/root/RenewGameState");var main=_main();var day:=int(state.get_value("player","day",1)) if state!=null else (int(main.get("day")) if main!=null else 1)
...
func _connect_history()->void:
    ...
    if history!=null and history.has_signal("gameplay_event_recorded"):
        var callable:=Callable(self,"_on_history_event")
        if not history.gameplay_event_recorded.is_connected(callable):history.gameplay_event_recorded.connect(callable)
func _on_history_event(event:Dictionary)->void:
    var event_day:=int(event.get("day",_last_day if _last_day>0 else 1))
    if _last_day<0:_last_day=event_day
    if current_issue.is_empty() or int(current_issue.get("day",-1))!=event_day:generate_daily(event_day)
    var story:=_history_story(event,event_day,0)
    if story.is_empty():return
    story["story_id"]="news_%06d"%(archive.size()+1)
    var stories:Array=current_issue.get("stories",[]).duplicate(true);stories.append(story)
    stories.sort_custom(func(a,b):return int(a.get("score",0))>int(b.get("score",0)))
    var kept:Array[Dictionary]=[];var section_counts:Dictionary={}
    for candidate in stories:
        var section:=str(candidate.get("section","Your Company"));var count:=int(section_counts.get(section,0));if count>=3:continue
        kept.append(candidate);section_counts[section]=count+1
        if kept.size()>=MAX_STORIES_PER_ISSUE:break
    current_issue["stories"]=kept
    _sync_archive_issue()
...
func _watch_day()->void:
    ...
    var day:=int(state.get_value("player","day",1)) if state!=null else int(main.get("day"))
    if _last_day<0:_last_day=day;return
    if day!=_last_day:_last_day=day;generate_daily(day)
func generate_daily(day:int)->Dictionary:
    _seen_sources.clear()
    var candidates:=_collect_events(day);candidates.sort_custom(func(a,b):return int(a.get("score",0))>int(b.get("score",0)))
    var stories:Array[Dictionary]=[];var used_sections:Dictionary={}
    for candidate in candidates:
        if stories.size()>=MAX_STORIES_PER_ISSUE:break
        var section:=str(candidate.get("section","Your Company"));var count:=int(used_sections.get(section,0));if count>=3:continue
    ...
    _sync_archive_issue();return current_issue.duplicate(true)
...
func get_issue(day:int)->Dictionary:
    ...
    for issue in archive:if int(issue.get("day",-1))==day:return issue.duplicate(true)
    return {}
...
func _main():var tree:=get_tree();if tree==null:return null;return tree.current_scene
