...
var next_contract_id: Variant = 1
...
func get_future_contract_offer(reputation: int, customer_id: String = DEFAULT_CUSTOMER) -> Dictionary:
    var relationship: Variant = _get_relationship(customer_id)
    ...
    var quantity: Variant = max(10, 25 + int(round(float(relationship - 50) * 0.2))); var base_price := max(120, 180 + reputation * 2 + int(round(float(relationship - 50) * 0.8))); var competitor_pressure := _competitor_bid_modifier(); var price := int(round(float(base_price) * (1.0 + competitor_pressure))); var duration := 5 if relationship >= 60 else 4
...
func _new_id() -> String:
    var value: Variant = "contract_%04d" % next_contract_id; next_contract_id += 1; return value
func create_customer_contract(parties: Array, product: String, quantity: int, price: int, quality_requirement: int, delivery_schedule: Dictionary, destination: String, penalty: int, cancellation: Dictionary, renewal: Dictionary, reputation_impact: Dictionary, kind: String = "standard") -> Dictionary:
    ...
    var customer_id: Variant = _customer_id(parties)
...
func execute_day(contract_id: String, available_quantity: int, average_quality: int, day: int) -> Dictionary:
    ...
    var frequency: Variant = str(schedule.get("frequency", "daily")); var delivery_day := true
    ...
    var daily_due: Variant = max(0, min(int(contract["quantity"]) - int(contract["quantity_due"]), int(schedule.get("quantity_per_delivery", contract["quantity"]))))
    var delivered: Variant = min(daily_due, max(0, available_quantity)); var quality_ok := average_quality >= int(contract["quality_requirement"])
...
func cancel_contract(contract_id: String, reason: String, day: int) -> Dictionary:
    ...
    var customer_id: Variant = str(contract.get("customer_id", DEFAULT_CUSTOMER)); var before := _get_relationship(customer_id); _set_relationship(customer_id, before + int(contract.get("reputation_impact", {}).get("on_cancelled", -4))); contract["customer_relationship_change"] = _get_relationship(customer_id) - before
...
func renew_contract(contract_id: String, day: int) -> Dictionary:
    ...
    var customer_id: Variant = str(source.get("customer_id", DEFAULT_CUSTOMER)); var relationship := _get_relationship(customer_id)
    ...
    var result: Variant = create_customer_contract(source["parties"], str(source["resource_product"]), int(source["quantity"]), new_price, int(source["quality_requirement"]), schedule, str(source["destination"]), int(source["penalty"]), source["cancellation"], source["renewal"], source["reputation_impact"])
    if bool(result.get("ok", false)):
        var new_id: Variant = str(result["contract"]["id"])
        for item in completed_contracts:
...
func _finish(contract: Dictionary) -> void:
    var due: Variant = max(1, int(contract["quantity_due"])); var delivered := int(contract["quantity_delivered"]); var fulfilment := float(delivered) / float(due); var status := "fulfilled" if fulfilment >= 0.999 and int(contract["missed_deliveries"]) == 0 else "breached"; contract["execution_status"] = status
    var customer_id: Variant = str(contract.get("customer_id", DEFAULT_CUSTOMER)); var impact: Dictionary = contract.get("reputation_impact", {}); var relationship_delta := int(impact.get("on_fulfilled", 10)) if status == "fulfilled" else int(impact.get("on_failed", -20)); var before := _get_relationship(customer_id)
    _set_relationship(customer_id, before + relationship_delta); contract["customer_relationship_before"] = before; contract["customer_relationship_after"] = _get_relationship(customer_id); contract["customer_relationship_change"] = relationship_delta; contract["renewal_offered"] = status == "fulfilled" and bool(contract["renewal"].get("eligible", false)) and _get_relationship(customer_id) >= 50
