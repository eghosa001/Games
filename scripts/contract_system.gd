...
func _customer_id(parties: Array) -> String:
    return str(parties[1]) if parties.size() >= 2 else DEFAULT_CUSTOMER
func _get_relationship(customer_id: String) -> int:
    return clampi(int(customer_relationships.get(customer_id, INITIAL_RELATIONSHIP)), 0, 100)
func _set_relationship(customer_id: String, value: int) -> void:
    customer_relationships[customer_id] = clampi(value, 0, 100)
...
func create_customer_contract(parties: Array, product: String, quantity: int, price: int, quality_requirement: int, delivery_schedule: Dictionary, destination: String, penalty: int, cancellation: Dictionary, renewal: Dictionary, reputation_impact: Dictionary) -> Dictionary:
    ...
    if _get_relationship(customer_id) < 20:
        return {"ok": false, "message": "Customer relationship is too weak for this contract."}
    ...
    var id := _new_id()
    var contract := {
        "id": id, "parties": parties.duplicate(true), "customer_id": customer_id,
        "resource_product": product, "quantity": quantity, "price": price,
        "quality_requirement": quality_requirement, "delivery_schedule": delivery_schedule.duplicate(true),
        "destination": destination, "penalty": penalty, "cancellation": cancellation.duplicate(true),
        "renewal": renewal.duplicate(true), "reputation_impact": reputation_impact.duplicate(true),
        "execution_status": "active", "signed_day": int(delivery_schedule.get("start_day", 1)),
        "days_elapsed": 0, "quantity_delivered": 0, "quantity_due": 0, "quality_delivered": 0,
...
func create_default_customer_contract(day: int, reputation: int) -> Dictionary:
    ...
    var quantity := int(offer.get("quantity", 25)); var duration := int(offer.get("duration_days", 5))
    var per_delivery := int(ceil(float(quantity) / float(duration)))
...
func execute_day(contract_id: String, available_quantity: int, average_quality: int, day: int) -> Dictionary:
    ...
    contract["days_elapsed"] = int(contract["days_elapsed"]) + 1; contract["quantity_due"] = int(contract["quantity_due"]) + daily_due
    contract["quantity_delivered"] = int(contract["quantity_delivered"]) + delivered; contract["quality_delivered"] = max(int(contract["quality_delivered"]), average_quality)
    contract["revenue_earned"] = int(contract["revenue_earned"]) + revenue; contract["penalties_paid"] = int(contract["penalties_paid"]) + penalty
...
func cancel_contract(contract_id: String, reason: String, day: int) -> Dictionary:
    ...
    contract["execution_status"] = "cancelled"; contract["cancel_reason"] = reason
    var fee := int(terms.get("fee", 0)); contract["penalties_paid"] = int(contract["penalties_paid"]) + fee
    var customer_id := str(contract.get("customer_id", DEFAULT_CUSTOMER)); var before := _get_relationship(customer_id)
    _set_relationship(customer_id, before + int(contract.get("reputation_impact", {}).get("on_cancelled", -4)))
    contract["customer_relationship_change"] = _get_relationship(customer_id) - before
    _record("cancelled", contract_id, {"day": day, "reason": reason, "relationship": _get_relationship(customer_id)})
    active_contracts.erase(contract_id); completed_contracts.append(contract.duplicate(true)); _trim_completed()
    return {"ok": true, "contract": contract.duplicate(true), "penalty": fee, "customer_relationship": _get_relationship(customer_id), "relationship_change": contract["customer_relationship_change"]}
...
func renew_contract(contract_id: String, day: int) -> Dictionary:
    ...
    if source.is_empty() or str(source.get("execution_status", "")) != "fulfilled" or not bool(source.get("renewal_offered", false)):
        return {"ok": false, "message": "Contract is not eligible for renewal."}
...
func _finish(contract: Dictionary) -> void:
    var due := max(1, int(contract["quantity_due"])); var delivered := int(contract["quantity_delivered"]); var fulfilment := float(delivered) / float(due)
    var status := "fulfilled" if fulfilment >= 0.999 and int(contract["missed_deliveries"]) == 0 else "breached"; contract["execution_status"] = status
    var customer_id := str(contract.get("customer_id", DEFAULT_CUSTOMER)); var impact: Dictionary = contract.get("reputation_impact", {})
    var relationship_delta := int(impact.get("on_fulfilled", 10)) if status == "fulfilled" else int(impact.get("on_failed", -20)); var before := _get_relationship(customer_id)
    _set_relationship(customer_id, before + relationship_delta); contract["customer_relationship_before"] = before; contract["customer_relationship_after"] = _get_relationship(customer_id); contract["customer_relationship_change"] = relationship_delta
    contract["renewal_offered"] = status == "fulfilled" and bool(contract["renewal"].get("eligible", false)) and _get_relationship(customer_id) >= 60
    _record(status, contract["id"], {"fulfilment": fulfilment, "revenue": contract["revenue_earned"], "penalties": contract["penalties_paid"], "customer": customer_id, "relationship_change": relationship_delta, "relationship": _get_relationship(customer_id)})
    active_contracts.erase(contract["id"]); completed_contracts.append(contract.duplicate(true)); _trim_completed()
...
func capture_state() -> Dictionary:
    return {"system_version": SYSTEM_VERSION, "next_contract_id": next_contract_id, "active_contracts": active_contracts.duplicate(true), "completed_contracts": completed_contracts.duplicate(true), "history": history.duplicate(true), "customer_relationships": customer_relationships.duplicate(true)}
