...
## All financial/alliance mutations remain in the authoritative Alliance V1 API.
...
func _build() -> void:
    ...
    title = Label.new(); title.text = "ALLIANCE COUNCIL  •  RESTORATION NETWORK"; title.add_theme_font_size_override("font_size", 18); panel.add_child(title)
    ...
    _add_button("CREATE ALLIANCE", _create_alliance); _add_button("CONTRIBUTE $1,000", _contribute); _add_button("START RAILWAY", _project); _add_button("BUILD INFRA", _infra); _add_button("FUND RESEARCH", _research)
...
func _create_alliance() -> void:
    ...
    _message(String(alliance_system.create_alliance("RENEW Restoration Consortium", PLAYER_ID).get("message", "Alliance creation failed.")))
func _contribute() -> void:
    if alliance_system != null: _message(String(alliance_system.donate_money(PLAYER_ID, 1000).get("message", "Contribution could not be completed.")))
func _project() -> void:
    if alliance_system != null: _message(String(alliance_system.start_cooperative_project(PLAYER_ID, "Regional Railway Project").get("message", "Project could not be started.")))
func _infra() -> void:
    _message("Shared infrastructure is coordinated through the alliance network. Use the regional Infrastructure action to commit the physical asset.") if not _player_alliance().is_empty() else _message("Create an alliance before building shared infrastructure.")
func _research() -> void:
    _message("Shared research is queued for the alliance network. The technology council will unlock the next cooperative upgrade.") if not _player_alliance().is_empty() else _message("Create an alliance before funding shared research.")
...
func _refresh() -> void:
    ...
    var alliance := _player_alliance()
    if alliance.is_empty(): status.text = "No restoration consortium founded yet. Unite specialist corporations to share materials, logistics, research and infrastructure."; project.text = "NEXT  CREATE ALLIANCE  →  INVITE PARTNERS  →  START REGIONAL RAILWAY"; return
...
func _layout() -> void:
    ...
    visible_width = get_viewport().get_visible_rect().size.x; var narrow := visible_width < 760.0
    panel.position = Vector2(8,70) if narrow else Vector2(maxf(18.0, visible_width - 610.0), 92); panel.size = Vector2(maxf(304.0, visible_width - 16.0), maxf(440.0, get_viewport().get_visible_rect().size.y - 78.0)) if narrow else Vector2(590,430)
    title.position = Vector2(14,10); title.size = Vector2(panel.size.x - 100,28); close_button.position = Vector2(panel.size.x - 84,7); close_button.size = Vector2(72,44)
    status.position = Vector2(14,48); status.size = Vector2(panel.size.x - 28,68); project.position = Vector2(14,118); project.size = Vector2(panel.size.x - 28,50)
    var buttons: Array[Button] = []
    for child in panel.get_children():
        if child is Button and child != close_button: buttons.append(child)
    var y := 172.0; var cols := 2 if narrow else 3; var bw := (panel.size.x - 42.0) / float(cols)
    ...
    if partners_title != null: partners_title.position = Vector2(14, partner_y); partners_title.size = Vector2(panel.size.x - 28,18)
    partner_scroll.position = Vector2(14, partner_y + 22); partner_scroll.size = Vector2(panel.size.x - 28, 86); partner_box.custom_minimum_size = Vector2(maxf(panel.size.x - 28, 560.0), 82)
