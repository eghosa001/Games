...
func _refresh()->void:
    ...
    match active_tab:
        ...
        2:
            _button("NEXT RIVAL",_next_rival); _button("MAKE ALLIANCE OFFER",parent.make_alliance_offer); _button("IMPROVE RELATION",parent.improve_alliance); _button("SUPPLY DEAL",parent.propose_supply_deal); _button("CUSTOMER PARTNERSHIP",parent.propose_customer_partnership); _button("ACQUIRE TARGET",parent.negotiate_selected_acquisition); _button("TAKE LOAN",parent.take_loan); _button("REPAY LOAN",parent.repay_loan); _button("FINANCE STATUS",_show_finance_status); _screen("ALLIANCE COUNCIL","AlliancePanel"); _screen("DIPLOMACY","RenewDiplomacyUI"); _screen("TECHNOLOGY","TechnologyPanel")
        3:
            _screen("MARKET","MarketPanel"); _screen("INFRASTRUCTURE","HeadquartersPanel"); _screen("TECHNOLOGY","TechnologyPanel"); _screen("NEWS","NewsPanel"); _screen("HISTORY","HistoryPanel"); _screen("LIVE OPS","LiveOpsPanel"); _button("SAVE GAME",parent.save_game); _button("LOAD GAME",parent.load_game)
