...
func _layout() -> void:
    ...
    list_label.position = Vector2(side, 46)
    list_label.size = Vector2(width - side * 2.0, maxf(70.0, panel.size.y - 276.0))
    detail_label.position = Vector2(side, panel.size.y - 226.0)
    detail_label.size = Vector2(width - side * 2.0, 62.0)
    ...
    sell_button.position = Vector2(side + action_width + gap, y)
    ...
    for button in [next_button, sell_button, lease_button]:
...
func _refresh(_force: bool) -> void:
    ...
    for i in range(catalog.size()):
        ...
        if owned and i == selected:
            status = str(state.get_value("properties", "stage", "Neglected")).to_upper()
        var lease := ""
    ...
    if selected >= 0 and selected < catalog.size() and catalog[selected] is Dictionary:
        ...
        detail_label.text = "Select an asset to inspect its condition and capacity."
