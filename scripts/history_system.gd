...
const GAMEPLAY_EVENTS := ["FOUNDING","PROPERTY_ACQUIRED","PROPERTY_RESTORED","BUSINESS_OPENED","FIRST_PRODUCTION","FIRST_SALE","FIRST_PROFIT","EMPLOYEE_HIRED","EMPLOYEE_PROMOTED","CONTRACT_SIGNED",[...]
const EVENT_ALIASES := {"employee_milestone":"EMPLOYEE_PROMOTED","employee_promotion":"EMPLOYEE_PROMOTED","employee_hired":"EMPLOYEE_HIRED","property_acquired":"PROPERTY_ACQUIRED","property_restore[...]
const TYPES := ["founding","restoration","first_sale","first_profit","employee_milestone","contract","property_acquisition","expansion","resource_acquisition","alliance","acquisition","merger","cri[...]
var timeline:Array[Dictionary]=[]
var legacy:Dictionary={"founder":"","company_name":"RENEW","founded_day":1,"notable_events":0,"properties_acquired":0,"expansions":0,"alliances":0,"acquisitions":0,"contracts":0,"crises":0,"employ[...]
...
func record_gameplay_event(event_name:String,day:int,title:String,details:Dictionary={},signature:String="")->Dictionary:
	...
	var payload:=details.duplicate(true);payload["gameplay_event"]=canonical; return record(_mapped_type(canonical),day,title,payload,signature if not signature.is_empty() else "%s|%d|%s"%[canonic[...]
func record(event_type:String,day:int,title:String,details:Dictionary={},signature:String="",gameplay_event:String="")->Dictionary:
	var canonical:=str(EVENT_ALIASES.get(event_type,event_type));var kind:=_mapped_type(canonical) if GAMEPLAY_EVENTS.has(canonical) else (event_type if TYPES.has(event_type) else "general");var p[...]
	...
	var event:Dictionary={"id":"hist_%06d"%(timeline.size()+1),"day":max(1,day),"type":kind,"title":title,"details":payload,"signature":key,"importance":_importance(kind),"recorded_at":Time.get_da[...]
...
func record_employee_milestone(day:int,employee_id:String,employee_name:String,milestone:String,details:Dictionary={})->Dictionary:
	var payload:=details.duplicate(true);payload["employee_id"]=employee_id;payload["employee_name"]=employee_name;payload["milestone"]=milestone;var event_name:String="EMPLOYEE_PROMOTED" if miles[...]
...
func get_timeline(filter_type:String="",limit:int=100)->Array[Dictionary]:
	...
	for i in range(timeline.size()-1,start-1,-1):
		var event:Dictionary=timeline[i];if filter_type.is_empty() or str(event.get("type",""))==filter_type:result.append(event.duplicate(true))
	return result
...
func _mapped_type(canonical:String)->String:
	var mapped:Dictionary={"FOUNDING":"founding","PROPERTY_ACQUIRED":"property_acquisition","PROPERTY_RESTORED":"restoration","BUSINESS_OPENED":"founding","FIRST_PRODUCTION":"milestone","FIRST_SAL[...]
