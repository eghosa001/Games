...
func _build() -> void:
    scrim = ColorRect.new()
    scrim.color = Color(0.01, 0.05, 0.07, 0.78)
    ...
    title_label = _label("WORLD COMMAND", 18, TEXT)
    summary_label = _label("REGIONAL MARKETS", 9, MUTED)
    panel.add_child(title_label); panel.add_child(summary_label)

    close_button = _button("CLOSE", Callable(self, "_close"), 44)
    panel.add_child(close_button)

    region_scroll = ScrollContainer.new()
    region_scroll.horizontal_scroll_mode = ScrollContainer.SCROLL_MODE_DISABLED
    region_scroll.vertical_scroll_mode = ScrollContainer.SCROLL_MODE_AUTO
    panel.add_child(region_scroll)
    region_grid = GridContainer.new()
    region_grid.columns = 2
    region_grid.add_theme_constant_override("h_separation", 8)
    region_grid.add_theme_constant_override("v_separation", 8)
    region_scroll.add_child(region_grid)

    detail_label = _label("", 10, TEXT)
    ...
    _add_action("ESTABLISH", Callable(self, "_establish"))
    _add_action("INFRASTRUCTURE", Callable(self, "_infrastructure"))
    _add_action("SHIP", Callable(self, "_dispatch"))
    _add_action("TRADE ROUTE", Callable(self, "_trade"))

    feedback_label = _label("", 9, MUTED)
...
func _button(text: String, callback: Callable, min_height := 44) -> Button:
    ...
    var normal := _style(Color("142c33"), BORDER, 9)
    var hover := _style(Color("1b3b40"), ACCENT, 9)
...
func _add_action(text: String, callback: Callable) -> void:
    action_row.add_child(_button(text, callback, 46))
...
func _layout() -> void:
    ...
    var narrow := w < 720.0
    var margin := 10.0 if narrow else 28.0
    panel.position = Vector2(margin, margin + 8.0)
    panel.size = Vector2(w - margin * 2.0, h - margin * 2.0 - 8.0)
    title_label.position = Vector2(14, 10)
    title_label.size = Vector2(panel.size.x - 110, 28)
    summary_label.position = Vector2(14, 38)
    summary_label.size = Vector2(panel.size.x - 110, 18)
    close_button.position = Vector2(panel.size.x - 88, 10)
    close_button.size = Vector2(76, 44)
    region_scroll.position = Vector2(12, 62)
    region_scroll.size = Vector2(panel.size.x - 24, panel.size.y * (0.48 if narrow else 0.44))
    region_grid.columns = 1 if narrow else 2
    detail_label.position = Vector2(14, region_scroll.position.y + region_scroll.size.y + 8)
    detail_label.size = Vector2(panel.size.x - 28, 74 if narrow else 66)
    action_row.position = Vector2(12, detail_label.position.y + detail_label.size.y + 5)
    action_row.size = Vector2(panel.size.x - 24, 48)
    feedback_label.position = Vector2(14, action_row.position.y + 52)
    feedback_label.size = Vector2(panel.size.x - 28, panel.size.y - feedback_label.position.y - 10)
...
func _refresh() -> void:
    if controller == null:
        detail_label.text = "Regional command is unavailable."
        return
    var regions = controller.regions
    var list: Array = regions.regions
    ...
    for i in range(list.size()):
        ...
        var owned := int(regions.player_presence[i]) > 0
        var status := "LOCKED" if not unlocked else ("OPERATING" if owned else "AVAILABLE")
        var card := Button.new()
        card.text = "%s\nTIER %d  •  %s\nMARKET %.2fx  •  RIVALS %d  •  INFRA %d" % [str(data.get("name", "Region")), int(data.get("tier", 1)), status, float(regions.market_levels[i]), int(regions.rival_presence[i]), int(regions.infrastructure[i])]
        ...
        var active_style := _style(CARD_ACTIVE, ACCENT, 10) if i == selected_index else _style(CARD, BORDER, 10)
        card.add_theme_stylebox_override("normal", active_style)
        card.add_theme_stylebox_override("hover", _style(Color("1b3b40"), ACCENT, 10))
        card.add_theme_color_override("font_color", TEXT if unlocked else WARNING)
        region_grid.add_child(card)
    _update_detail()

func _select(index: int) -> void:
    if controller == null: return
    var result = controller.regions.select(index, int(game.reputation))
    if bool(result.get("ok", false)):
        selected_index = index
        feedback_label.text = str(result.get("message", "Region selected."))
        game._log("REGION: " + str(result.get("message", "Region selected.")))
    else:
        feedback_label.text = str(result.get("message", "Region selection unavailable."))
    _refresh()

func _update_detail() -> void:
    var r: Dictionary = controller.regions.current()
    var unlocked := bool(r.get("unlocked", false))
    var presence := int(r.get("player_presence", 0))
    var rivals := int(r.get("rival_presence", 0))
