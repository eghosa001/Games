...
func _layout() -> void:
    ...
    summary_label.size = Vector2(panel.size.x - 28 if phone else panel.size.x - 110, 18)
    summary_label.clip_text = true
    summary_label.text_overrun_behavior = TextServer.OVERRUN_TRIM_ELLIPSIS
    region_scroll.position = Vector2(12, 86 if phone else 62); region_scroll.size = Vector2(panel.size.x - 24, panel.size.y * (0.35 if phone else (0.43 if narrow else 0.44))); region_grid.columns = 1 if narrow else 2
