...
func _build_ui() -> void:
    ...
    dimmer.name = "HistoryModalScrim"
    dimmer.color = SCRIM
    dimmer.mouse_filter = Control.MOUSE_FILTER_STOP
    add_child(dimmer)

    panel = PanelContainer.new()
    panel.name = "CorporateMuseum"
    ...
    var title := Label.new()
    title.text = "RENEW CORPORATE MUSEUM"
    ...
    close_button.custom_minimum_size = Vector2(78, 46)
    ...
    status.text = "CORPORATE MEMORY  •  HISTORY & LEGACY"
    ...
    tabs_scroll.custom_minimum_size.y = 48
    ...
    scroll.name = "HistoryContentScroll"
...
func _add_tab(id: String, label_text: String) -> void:
    ...
    _style_tab(button, id == active_tab)
...
func _style_tab(button: Button, active: bool) -> void:
    ...
    button.add_theme_color_override("font_color", SURFACE if active else TEXT)
    button.add_theme_color_override("font_hover_color", SURFACE)
...
func _select_tab(id: String) -> void:
    ...
    for child in tabs.get_children():
        if child is Button:
            _style_tab(child, child.name == "Tab_%s" % id)
    _refresh()
...
func _refresh() -> void:
    ...
    if legacy == null:
        ...
        return
    var s: Dictionary = legacy.summary()
    var hq = _hq()
    var hq_text := "HQ: unavailable"
...
func _render_gallery(legacy) -> void:
    ...
    for category in ["founding", "products", "employees", "contracts", "acquisitions", "failures", "awards", "rankings", "technologies", "alliances", "crisis_recoveries"]:
        ...
        found = true
        _section(_category_title(category))
        _render_items(items, "")
...
func _render_items(items: Array, heading_text: String) -> void:
    ...
    if items.is_empty():
        _empty("No records yet. Major decisions will be preserved here.")
        return
    for item in items:
        ...
        var top := Label.new()
        top.text = "DAY %d  •  %s" % [int(item.get("day", 1)), _category_title(str(item.get("category", "general")))]
        top.add_theme_font_size_override("font_size", 9)
        top.add_theme_color_override("font_color", ACCENT)
        box.add_child(top)
        var title := Label.new()
        title.text = str(item.get("title", "Historic artifact"))
...
func _layout() -> void:
    ...
    var w := minf(720.0, maxf(280.0, size.x - margin * 2.0))
    var top := 36.0 if phone else 46.0
    var h := minf(720.0, maxf(360.0, size.y - top - 10.0))
    dimmer.position = Vector2.ZERO
    dimmer.size = size
    panel.position = Vector2((size.x - w) / 2.0, top)
    panel.size = Vector2(w, minf(h, size.y - top - 10.0))
    var compact := w < 420.0
    if close_button != null:
        close_button.custom_minimum_size = Vector2(72 if compact else 82, 46)
    if summary_label != null:
        summary_label.add_theme_font_size_override("font_size", 10 if compact else 11)
    for child in tabs.get_children():
        if child is Button:
            child.custom_minimum_size = Vector2(108 if compact else 118, 44)
