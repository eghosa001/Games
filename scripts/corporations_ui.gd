...
func _build_ui() -> void:
    ...
    status_label = _label("RIVAL INTELLIGENCE", 10, ACCENT)
...
func _layout() -> void:
    ...
    var width := minf(580.0, maxf(280.0, viewport.x - margin * 2.0))
    var height := minf(620.0, maxf(430.0, viewport.y - 96.0))
    ...
    title_label.size = Vector2(width - 150.0, 28)
    title_label.add_theme_font_size_override("font_size", 18 if compact else 20)
    status_label.position = Vector2(width - 132.0, 14)
    status_label.size = Vector2(118.0, 20)
    status_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_RIGHT
    status_label.add_theme_font_size_override("font_size", 9 if compact else 10)

    list_label.position = Vector2(side, 48)
    list_label.size = Vector2(width - side * 2.0, maxf(80.0, panel.size.y - 250.0))
    list_label.add_theme_font_size_override("font_size", 11 if compact else 12)
    detail_label.position = Vector2(side, panel.size.y - 194.0)
    detail_label.size = Vector2(width - side * 2.0, 64.0)
    detail_label.add_theme_font_size_override("font_size", 10 if compact else 11)

    var gap := 7.0
    var action_width := maxf(62.0, (width - side * 2.0 - gap * 3.0) / 4.0)
    var y := panel.size.y - 108.0
    next_button.position = Vector2(side, y)
    next_button.size = Vector2(action_width, 46)
    offer_button.position = Vector2(side + action_width + gap, y)
    offer_button.size = Vector2(action_width, 46)
    improve_button.position = Vector2(side + (action_width + gap) * 2.0, y)
    improve_button.size = Vector2(action_width, 46)
    shares_button.position = Vector2(side + (action_width + gap) * 3.0, y)
    shares_button.size = Vector2(action_width, 46)
    for button in [next_button, offer_button, improve_button, shares_button]:
...
func _refresh(_force: bool) -> void:
    ...
    status_label.text = "%d RIVAL%s" % [count, "" if count == 1 else "S"]
    if selected >= 0 and selected < count:
        ...
        detail_label.text = "Select a corporation to inspect its relationship and share price."
