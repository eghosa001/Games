...
var phase := EXPANSION
var phase_strength := 0.0
var phase_start_day := 1
var cycle_index := 0
var indicators := {"demand":1.0,"credit":1.0,"investment":1.0,"employment":1.0,"inflation":1.0}
...
var phase_targets := {"boom":1.20,"expansion":1.10,"overheating":1.02,"recession":0.76,"recovery":1.05}
var last_day := -1
var history: Array = []
...
func process_day(day: int, context: Dictionary = {}) -> Dictionary:
    ...
    last_day = day
    var signal := _economic_signal(context)
...
func _advance_phase(signal: float, day: int) -> void:
    var duration := day - phase_start_day
    var next := phase
    match phase:
...
func _economic_signal(context: Dictionary) -> float:
    var demand := float(context.get("demand", indicators["demand"]))
    var investment := float(context.get("investment", indicators["investment"]))
    var credit := float(context.get("credit", indicators["credit"]))
    var inflation := float(context.get("inflation", indicators["inflation"]))
    return demand * 0.40 + investment * 0.30 + credit * 0.20 + (2.0 - inflation) * 0.10

func _update_indicators() -> void:
    var target := float(phase_targets[phase])
