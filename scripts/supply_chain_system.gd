...
var warehouse: Dictionary = {
    "timber": 0.0, "iron": 0.0, "metal": 0.0, "energy": 0.0,
    "food": 0.0, "electronics": 0.0, "furniture": 0.0
}
...
func _transport_capacity(level: int) -> float:
    return 40.0 + max(0, level - 1) * 20.0
...
func _freight_cost(resource: String, amount: float, level: int) -> int:
    ...
    if economy != null and economy.resources.has(resource):
        var region := str(economy.resources[resource].get("region", "Unknown"))
        distance_factor = 1.0 + float(region.length() % 4) * 0.05
    return int(round(amount * (2.0 + float(max(0, 3 - level))) * distance_factor))

# Buys real market stock and physically moves it into the company warehouse.
func procure(resource: String, amount: float, cash: int, transport_level: int = 1) -> Dictionary:
    ...
    var capacity := _transport_capacity(transport_level)
    if amount > capacity:
        return {"ok": false, "reason": "transport_capacity", "capacity": capacity, "requested": amount}
    var available := float(economy.resources[resource].get("stock", 0.0))
    if available < amount:
        return {"ok": false, "reason": "market_stock", "available": available, "requested": amount}
    var unit_price := float(economy.current_price(resource))
    var material_cost := int(round(unit_price * amount))
    var freight := _freight_cost(resource, amount, transport_level)
    var total := material_cost + freight
    if cash < total:
        return {"ok": false, "reason": "cash", "cost": total}
    economy.resources[resource]["stock"] = max(0.0, available - amount)
    warehouse[resource] = min(WAREHOUSE_LIMIT, stock(resource) + amount)
    total_freight_cost += freight
    last_operation = {"type": "procure", "resource": resource, "amount": amount, "material_cost": material_cost, "freight": freight, "cost": total, "region": economy.resources[resource].get("region", "Unknown")}
    return {"ok": true, "resource": resource, "amount": amount, "cost": total, "material_cost": material_cost, "freight": freight}

func procure_bundle(orders: Array, cash: int, transport_level: int = 1) -> Dictionary:
    var total_amount := 0.0
    for order in orders:
        total_amount += float(order.get("amount", 0.0))
    if total_amount > _transport_capacity(transport_level):
        return {"ok": false, "reason": "transport_capacity", "capacity": _transport_capacity(transport_level), "requested": total_amount}
    var total_cost := 0
    for order in orders:
        var resource := str(order.get("resource", "")); var amount := float(order.get("amount", 0.0))
    ...
    var delivered: Array = []
    for order in orders:
        var result := procure(str(order["resource"]), float(order["amount"]), cash, transport_level)
        if not result["ok"]: return {"ok": false, "reason": "delivery_failed", "details": result}
        cash -= int(result["cost"]); delivered.append(result)
    return {"ok": true, "cost": total_cost, "delivered": delivered, "warehouse": warehouse_snapshot()}

# Iron -> Metal Factory -> Metal. Metal is an intermediate, not a sixth market resource.
func process_iron_to_metal(cycles: int = 1) -> Dictionary:
    cycles = max(0, cycles)
    var possible := cycles
    possible = min(possible, int(floor(stock("iron") / METAL_RECIPE["iron"])))
...
func can_make_furniture(cycles: int = 1) -> Dictionary:
    var possible := max(0, cycles)
    for resource in FURNITURE_INPUTS:
        var per_cycle := float(FURNITURE_INPUTS[resource])
        possible = min(possible, int(floor(stock(resource) / per_cycle)))
...
func consume_furniture_inputs(cycles: int) -> Dictionary:
    var check := can_make_furniture(cycles)
...
func sell_furniture(amount: int, price: int) -> Dictionary:
    var sold := min(max(0, amount), int(floor(stock("furniture"))))
    var revenue := sold * max(0, price)
...
func restore_state(snapshot: Dictionary) -> void:
    ...
    warehouse = snapshot.get("warehouse", warehouse).duplicate(true)
