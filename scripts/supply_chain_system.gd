...
func procure(resource:String,amount:float,cash:int,transport_level:int=1)->Dictionary:
    ...
    economy.resources[resource]["stock"]=max(0.0,available-amount);warehouse[resource]=min(WAREHOUSE_LIMIT,stock(resource)+delivered_amount);total_freight_cost+=freight;last_operation={"type":"procure","resource":resource,"amount":amount,"delivered_amount":delivered_amount,"material_cost":material_cost,"freight":freight,"cost":total,"region":economy.resources[resource].get("region","Unknown")};return {"ok":true,"resource":resource,"amount":amount,"delivered_amount":delivered_amount,"cost":total,"material_cost":material_cost,"freight":freight}
func procure_bundle(orders:Array,cash:int,transport_level:int=1)->Dictionary:
    ...
    var delivered:Array=[];var remaining_cash:=cash
    for order in orders:
        var result:=procure(str(order["resource"]),float(order["amount"]),remaining_cash,transport_level);if not result["ok"]:return {"ok":false,"reason":"delivery_failed","details":result};remaining_cash-=int(result["cost"]);delivered.append(result)
