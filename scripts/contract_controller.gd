...
func _process(_delta: float) -> void:
    ...
    var current_day := _main_day()
    if current_day == observed_day: return
    if current_day < observed_day:
        observed_day = current_day
        return
    var contract := contract_system.active_contract(observed_contract_id)
    if contract.is_empty():
        observed_contract_id = ""
        observed_day = current_day
        return
    var production_quality := _quality()
    var available := int(main.get("finished_goods"))
    var result := contract_system.execute_day(observed_contract_id, available, production_quality, current_day)
    if not bool(result.get("ok", false)):
        observed_day = current_day
        return
    var delivered := int(result.get("delivered", 0))
    ...
    if bool(result.get("finished", false)):
        ...
        var rep_change := int(impact.get("on_fulfilled", 0)) if status == "fulfilled" else int(impact.get("on_failed", 0))
