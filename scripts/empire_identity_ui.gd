...
func _label(text: String, size: int, color: Color) -> Label:
    var l := Label.new(); l.text = text; l.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART; l.add_theme_font_size_override("font_size", size); l.add_theme_color_override("font_color", color); return l
func _button(text: String, callback: Callable) -> Button:
    var b := Button.new(); b.text = text; b.custom_minimum_size = Vector2(88, 46); b.focus_mode = Control.FOCUS_NONE; b.add_theme_font_size_override("font_size", 10); b.add_theme_color_override("font_color", TEXT); b.add_theme_stylebox_override("normal", _style(SURFACE_2)); b.add_theme_stylebox_override("hover", _style(SURFACE_3, ACCENT)); b.add_theme_stylebox_override("pressed", _style(SURFACE_3, ACCENT)); b.pressed.connect(callback); return b

func _build() -> void:
    dimmer = ColorRect.new(); dimmer.name = "IdentityModalScrim"; dimmer.color = SCRIM; dimmer.mouse_filter = Control.MOUSE_FILTER_STOP; add_child(dimmer)
    panel = Panel.new(); panel.name = "EmpireIdentitySurface"; panel.mouse_filter = Control.MOUSE_FILTER_STOP; panel.add_theme_stylebox_override("panel", _style(SURFACE, BORDER, 14)); add_child(panel)
