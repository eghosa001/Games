...
# V2 adds an in-panel recruitment action and authoritative feedback so the
# People Operations screen can complete the hire -> manage loop without
# forcing the player back to the main dashboard.
...
func _build_ui() -> void:
    ...
    _feedback.add_theme_color_override("font_color", GOOD)
    ...
    _add_action(_actions, "HIRE", "hire")
...
func _action_hint(action: String) -> String:
    match action:
        "hire": return "Recruit the first available candidate using the authoritative hiring system."
...
func _refresh() -> void:
    ...
    for employee in roster:
        if str(employee.get("status", "active")) == "active":
            active_count += 1
    var candidates := _candidate_count()
    var daily_wages := _daily_wages()
...
func _rebuild_list(roster: Array) -> void:
    ...
    for employee in roster:
        ...
        var assignment := str(employee.get("assignment", ""))
...
func _update_details(roster: Array) -> void:
    ...
    if employee.is_empty():
        detail_label.text = "No employee selected.\nUse HIRE to recruit the first available candidate."
    ...
    detail_label.text = "%s\n%s  •  %s\nProductivity %d%%  •  Experience %d\nMorale %d  •  Loyalty %d\nSalary $%d/month  •  Assignment: %s\nSpecialization: %s" % [str(employee.get("name", "Employee")), str(employee.get("role", "Worker")), seat_line, productivity, int(employee.get("experience", 0)), int(employee.get("morale", 0)), int(employee.get("loyalty", 0)), int(employee.get("salary", 0)), str(employee.get("assignment", "unassigned")), str(employee.get("specialization", "general")).capitalize()]
...
func _action(action: String) -> void:
    ...
    if command == null:
        _feedback.text = "People Operations is temporarily unavailable."
        return
    if action == "hire":
...
func _candidate_count() -> int:
    var command = _command_system()
    if command == null or command.employee_system == null:
        return 0
    if command.employee_system.has_method("get_candidates"):
        return command.employee_system.get_candidates().size()
    return 0

func _daily_wages() -> int:
    var command = _command_system()
    if command == null or command.employee_system == null:
        return 0
    if command.employee_system.has_method("get_daily_wage_total"):
        return int(command.employee_system.get_daily_wage_total())
    return 0
