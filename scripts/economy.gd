...
func _ensure_resource(resource:String,config:Dictionary)->void:
    ...
    d["current_price"]=clamp(float(d.get("current_price",d.get("price",d["base_price"]))),MIN_PRICE,MAX_PRICE); d["price"]=d["current_price"]
    d["production_rate"]=float(d.get("production_rate",config["production_rate"])); d["consumption_rate"]=float(d.get("consumption_rate",config["consumption_rate"]))
...
func supplier_for(resource:String,choice:int=0)->Dictionary:
    ...
    var list:Array=suppliers[resource]; return list[clamp(choice,0,list.size()-1)]
func set_market_modifier(resource:String,multiplier:float)->void:
    if market_multipliers.has(resource): market_multipliers[resource]=clamp(multiplier,0.65,1.60)
...
func availability(resource:String)->float:
    ...
    var d:Dictionary=resources[resource]; return clamp(float(d.get("stock",0.0))/max(1.0,float(d.get("target_stock",100.0))),0.0,2.0)
func scarcity(resource:String)->Dictionary:
    if not resources.has(resource): return {"ok":false}
    var d:Dictionary=resources[resource]; var a:float=availability(resource); var shortage:float=clamp(1.0-a,-1.0,1.0); var severity:float=clamp(shortage,0.0,1.0)
    d["scarcity"]=severity; resources[resource]=d; var factor:float=clamp(1.0+shortage*1.15,0.55,2.50); var production_factor:float=clamp(1.0-severity*0.55,0.45,1.0); var status:String="surplus"
...
func production_cost(orders:Array,choice:int=0)->Dictionary:
    ...
    for order in orders:
        ...
        var q:Dictionary=quote(String(order.get("resource","")),int(order.get("amount",0)),choice); if not q["ok"]: return {"ok":false,"cost":0,"quotes":[]}; total+=int(q["cost"]); quotes.append(q)
    return {"ok":true,"cost":total,"quotes":quotes}
func buy_resource(resource:String,amount:int,cash:int,choice:int=0)->Dictionary:
    var q:Dictionary=quote(resource,amount,choice); if not q["ok"] or cash<q["cost"]: return {"ok":false,"cost":q["cost"],"amount":0,"supplier":q.get("supplier","Unknown")}
    var supplier:Dictionary=supplier_for(resource,choice); if randi_range(1,100)>int(supplier["reliability"]): return {"ok":false,"cost":0,"amount":0,"supplier":supplier["name"],"failed":true}
    resources[resource]["stock"]+=amount; add_demand(resource,float(amount)); return {"ok":true,"cost":q["cost"],"amount":amount,"supplier":supplier["name"]}
...
func register_customer_demand(product:String,units:float)->Dictionary:
    ...
    var recipe:Dictionary=CUSTOMER_INPUT_DEMAND.get(product,{}); if recipe.is_empty(): return {"ok":false,"units":units,"reason":"unknown_product"}
    var added:Dictionary={}; for resource in recipe: var amount:float=float(recipe[resource])*units; add_demand(String(resource),amount); added[String(resource)]=amount
...
func estimate_demand(resource:String,market_price:float)->float:
    if not resources.has(resource): return 0.0
    var base:float=float(resources[resource]["base_price"]); return max(0.0,consumption_rate(resource)*(1.0-clamp((market_price/base-1.0)*0.8,-0.7,0.9)))
...
func _recalculate_price(resource:String)->void:
    if not resources.has(resource): return
    var d:Dictionary=resources[resource]; var supply_value:float=max(0.1,float(d.get("supply",0.0))); var demand_value:float=max(0.0,float(d.get("demand",0.0))); var ratio:float=demand_value/supply_value; var scarcity_component:float=clamp(ratio,0.55,2.50); var modifier:float=clamp(float(market_multipliers.get(resource,1.0)),0.65,1.60); var target_price:float=clamp(float(d["base_price"])*pow(scarcity_component,0.70)*modifier,MIN_PRICE,MAX_PRICE); var current:float=float(d.get("current_price",d["base_price"])); d["current_price"]=clamp(current+(target_price-current)*PRICE_RESPONSE,MIN_PRICE,MAX_PRICE); d["price"]=d["current_price"]; d["scarcity"]=clamp(1.0-float(d.get("stock",0.0))/max(1.0,float(d.get("target_stock",100.0))),0.0,1.0); resources[resource]=d
