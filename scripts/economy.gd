...
# Phase 10: Economy is the single authority for market resource supply,
# demand and price. Other systems may transact with the market, but they do
# not invent independent resource prices.
    ...
    for resource in RESOURCE_CONFIG:
        if not resources.has(resource):
            ...
            resources[resource] = {
                ...
                "price": c["base_price"],
                "supply": c["production_rate"],
                "demand": c["consumption_rate"],
                "production_rate": c["production_rate"],
                "consumption_rate": c["consumption_rate"],
                "stock": c["stock"],
                "scarcity": 0.0,
                "region": c["region"],
                "target_stock": c["stock"]
            }
...
func _default_suppliers(resource:String)->Array:
    return [
        {"name":"Regional Producer","reliability":90,"markup":1.00,"region":String(resources.get(resource,{}).get("region","Unknown"))},
        {"name":"National Broker","reliability":76,"markup":1.12,"region":"National"},
        {"name":"Emergency Importer","reliability":60,"markup":1.30,"region":"International"}
    ]
...
func quote(resource:String,amount:int,choice:int=0)->Dictionary:
    ...
    var market_price:=current_price(resource)
    var cost:=int(round(market_price*amount*float(supplier["markup"])*market_factor))
...
# Demand is owned by Economy. Callers can provide real market demand changes
# without setting a price. Price is always derived from supply versus demand.
func set_demand(resource:String,value:float)->void:
    if not resources.has(resource): return
    resources[resource]["demand"] = max(0.0,value)
    _recalculate_price(resource)
...
func _recalculate_price(resource:String)->void:
    if not resources.has(resource): return
    var d:Dictionary=resources[resource]
    var supply_value:=max(0.1,float(d.get("supply",0.0)))
    var demand_value:=max(0.0,float(d.get("demand",0.0)))
    var ratio:=demand_value/supply_value
    var scarcity_component:=clamp(ratio,0.55,2.50)
    var modifier:=clamp(float(market_multipliers.get(resource,1.0)),0.55,2.50)
    var target_price:=clamp(float(d["base_price"])*pow(scarcity_component,0.70)*modifier,MIN_PRICE,MAX_PRICE)
    var current:=float(d.get("current_price",d["base_price"]))
    d["current_price"]=clamp(current+(target_price-current)*PRICE_RESPONSE,MIN_PRICE,MAX_PRICE)
    d["price"]=d["current_price"]
    d["scarcity"]=clamp(1.0-float(d.get("stock",0.0))/max(1.0,float(d.get("target_stock",100.0))),0.0,1.0)
    resources[resource]=d
...
func end_market_day()->void:
    ...
    for key in resources:
        ...
        var stock:=max(0.0,float(d.get("stock",0.0)))
        var production:=max(0.0,float(d.get("production_rate",0.0)))
        var baseline_demand:=max(0.0,float(d.get("consumption_rate",0.0)))

        # Supply is the actual market quantity available for the day. Stock
        # affects next-day availability; production adds new supply.
        var daily_supply:=max(0.0,production+randi_range(-2,4))
        var daily_demand:=estimate_demand(String(key),current_price(String(key)))
        d["supply"]=daily_supply
        d["demand"]=daily_demand
        stock=max(0.0,stock+daily_supply-daily_demand)
        d["stock"]=stock
        d["consumption_rate"]=baseline_demand
        resources[key]=d
        _recalculate_price(String(key))
