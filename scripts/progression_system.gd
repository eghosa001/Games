...
func sync_tracking() -> void:
    var state=_state(); if state==null:return
    _last_restoration=int(state.get_value("properties","restoration",0))
    _last_profit=int(state.get_value("economy","total_profit",0))
    var roster=state.get_value("employees","roster",[]);_last_employees=roster.size() if roster is Array else 0
    ...
    var expansion=state.get_value("branches","expansion",{});_last_expansion_count=expansion.size() if expansion is Dictionary else 0;_last_expansion_signature=str(expansion)
    _last_operational=str(state.get_value("properties","stage",""))=="Operational"
...
func _track_meaningful_progress() -> void:
    ...
    if restoration>_last_restoration:
        award_action("restoration_step");_last_restoration=restoration
    var operational:=str(state.get_value("properties","stage",""))=="Operational"
    if operational and not _last_operational:
        award_action("property_operational");_last_operational=true
    elif not operational:
        _last_operational=false
    var total_profit:=int(state.get_value("economy","total_profit",0))
    if total_profit>_last_profit:
        award_profit(total_profit-_last_profit);_last_profit=total_profit
    var roster=state.get_value("employees","roster",[]);var employees:=roster.size() if roster is Array else 0
    if employees>_last_employees: award_action("employee_hired",employees-_last_employees)
    _last_employees=employees
    var goods:=int(state.get_value("production","finished_goods",0))
    if goods>_last_goods: award_action("production_run");_last_goods=goods
    elif goods<_last_goods: _last_goods=goods
    var contract_days:=int(state.get_value("contracts","contract_days",0))
    if _last_contract_days<=0 and contract_days>0: award_action("contract_signed")
    elif _last_contract_days>0 and contract_days<=0: award_action("contract_completed")
    _last_contract_days=contract_days
    var expansion=state.get_value("branches","expansion",{})
    if expansion is Dictionary:
        var count:=expansion.size();var signature:=str(expansion)
