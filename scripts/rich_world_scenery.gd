...
func _process(delta: float) -> void:
    elapsed += delta
    redraw_clock += delta
    if redraw_clock >= 0.25:
        redraw_clock = 0.0
        queue_redraw()

func _draw() -> void:
    var size: Vector2 = get_viewport_rect().size
    var w: float = maxf(size.x, 360.0)
    var h: float = maxf(size.y, 568.0)
    var compact: bool = w < 720.0
    _sky(w, h)
    _ground(w, h, compact)
    _campus(w, h, compact)
    _districts(w, h, compact)
    _public_realm(w, h)
    _life(w, h)
    _badges(w, h, compact)

func _sky(w: float, h: float) -> void:
    var horizon: float = h * 0.31
    for i in range(8):
        var t: float = float(i) / 7.0
        draw_rect(Rect2(0.0, t * horizon, w, horizon / 7.0 + 2.0), SKY_A.lerp(SKY_B, t), true)
    _glow(Vector2(w * 0.79, horizon * 0.36), minf(250.0, w * 0.28), GOLD, 0.08)
...
func _ground(w: float, h: float, compact: bool) -> void:
    var top: float = h * 0.30
    var bottom: float = h * 0.98
    var cx: float = w * 0.5
    var ht: float = w * (0.47 if compact else 0.43)
    var hb: float = w * (0.64 if compact else 0.55)
    var island := PackedVector2Array([Vector2(cx-ht,top),Vector2(cx+ht,top),Vector2(cx+hb,bottom),Vector2(cx-hb,bottom)])
    ...
    draw_colored_polygon(PackedVector2Array([Vector2(cx-ht,road_y-30.0),Vector2(cx+ht,road_y-30.0),Vector2(cx+hb,road_y+38.0),Vector2(cx-hb,road_y+38.0)]), ROAD)
    draw_line(Vector2(cx-ht,road_y-30.0), Vector2(cx+ht,road_y-30.0), EDGE, 2.0)
    draw_line(Vector2(cx-hb,road_y+38.0), Vector2(cx+hb,road_y+38.0), EDGE, 2.0)
    for x in range(int(cx-ht+28.0), int(cx+ht-20.0), 78):
        draw_line(Vector2(float(x),road_y+3.0), Vector2(float(x+34),road_y+3.0), Color(GOLD.r,GOLD.g,GOLD.b,0.68), 2.0)
    draw_colored_polygon(PackedVector2Array([Vector2(cx-38.0,top+4.0),Vector2(cx+38.0,top+4.0),Vector2(cx+108.0,bottom),Vector2(cx-108.0,bottom)]), Color("30464d"))

func _campus(w: float, h: float, compact: bool) -> void:
    var s: float = clampf(w / 1280.0, 0.62, 1.08)
    if compact:
        s *= 0.90
    var c := Vector2(w*0.50, h*0.49)
    _plate(c+Vector2(0.0,44.0*s), 438.0*s, 214.0*s, 16.0*s, Color("e8dfca"), WALL)
    _building(c+Vector2(-158.0,-4.0)*s, Vector2(108.0,68.0)*s, 48.0*s, Color("e7e1d4"), GLASS, "FINANCE", s)
    _building(c+Vector2(158.0,-4.0)*s, Vector2(108.0,68.0)*s, 48.0*s, Color("e7e1d4"), Color("8fbbb0"), "PRODUCT", s)
    _building(c+Vector2(0.0,-67.0)*s, Vector2(134.0,70.0)*s, 50.0*s, Color("efe6d5"), Color("d39c66"), "STRATEGY", s)
    for row in range(2):
        for col in range(4):
            _desk(c+Vector2(-122.0+float(col)*80.0,48.0+float(row)*57.0)*s, s, row*4+col)
    _lounge(c+Vector2(-150.0,112.0)*s, s)
    _meeting(c+Vector2(146.0,110.0)*s, s)
    _brand(c+Vector2(0.0,-145.0)*s, s)
    _tower(c+Vector2(0.0,-184.0)*s, s)

func _districts(w: float, h: float, compact: bool) -> void:
    var s: float = clampf(w / 1280.0, 0.60, 1.0)
    var y: float = h * (0.79 if compact else 0.80)
    _building(Vector2(w*0.18,y), Vector2(144.0,64.0)*s, 42.0*s, Color("c2aa86"), ORANGE, "MANUFACTURING", s)
    _building(Vector2(w*0.38,y+14.0*s), Vector2(132.0,58.0)*s, 35.0*s, Color("d0c1a6"), Color("6e9d9d"), "LOGISTICS", s)
    _building(Vector2(w*0.69,y), Vector2(124.0,60.0)*s, 50.0*s, Color("dbe4df"), GLASS, "RESEARCH", s)
    _building(Vector2(w*0.85,y+18.0*s), Vector2(116.0,55.0)*s, 37.0*s, Color("d9c3a6"), ORANGE, "RETAIL", s)

func _public_realm(w: float, h: float) -> void:
    var s: float = clampf(w / 1280.0, 0.60, 1.0)
    for i in range(9):
        _tree(Vector2(w*(0.08+0.105*float(i)), h*0.66+float(i%2)*17.0*s), s*(0.84+0.06*float(i%3)))
    for i in range(6):
        _lamp(Vector2(w*(0.14+0.145*float(i)), h*0.705), s)
    _fountain(Vector2(w*0.50,h*0.69), s)

func _life(w: float, h: float) -> void:
    ...
    for i in range(starts.size()):
        var t: float = fmod(elapsed*(0.045+0.007*float(i))+float(i)*0.21,1.0)
        _person(starts[i].lerp(ends[i],t), s*(0.86+0.05*float(i)), i)
    for i in range(3):
        var t: float = fmod(elapsed*(0.055+0.006*float(i))+float(i)*0.31,1.0)
        var travel: float = t if i%2==0 else 1.0-t
        _vehicle(Vector2(lerpf(w*0.10,w*0.90,travel),h*0.625+float(i)*7.0),s*0.82,i)

func _badges(w: float, h: float, compact: bool) -> void:
    if compact:
        return
    var day: int = 1
    var cash: int = 0
...
func _plate(c: Vector2, width: float, depth: float, height: float, top: Color, side: Color) -> void:
    ...
    var p := PackedVector2Array([c+Vector2(0.0,-hd),c+Vector2(hw,0.0),c+Vector2(0.0,hd),c+Vector2(-hw,0.0)])
    draw_colored_polygon(PackedVector2Array([p[3],p[2],p[2]+Vector2(0.0,height),p[3]+Vector2(0.0,height)]),side.darkened(0.12))
    draw_colored_polygon(PackedVector2Array([p[1],p[2],p[2]+Vector2(0.0,height),p[1]+Vector2(0.0,height)]),side.darkened(0.27))
    draw_colored_polygon(p,top)

func _building(c: Vector2, footprint: Vector2, height: float, wall: Color, accent: Color, label: String, s: float) -> void:
    _plate(c,footprint.x,footprint.y,5.0*s,wall.lightened(0.04),WALL)
...
func _desk(p: Vector2, s: float, variant: int) -> void:
    ...
    _plate(p,52.0*s,26.0*s,4.0*s,wood,wood.darkened(0.22))
...
func _lounge(p: Vector2, s: float) -> void:
    _plate(p,116.0*s,64.0*s,4.0*s,Color("d6c7aa"),Color("9c8d75"))
    _plate(p+Vector2(-26.0,4.0)*s,44.0*s,24.0*s,8.0*s,Color("5f8079"),Color("3f5d58"))
    _plate(p+Vector2(30.0,4.0)*s,44.0*s,24.0*s,8.0*s,Color("7e6d66"),Color("574b47"))

func _meeting(p: Vector2, s: float) -> void:
    _plate(p,120.0*s,70.0*s,4.0*s,Color("e1d5bb"),Color("a89b81"))
    draw_circle(p,24.0*s,Color("c7995d"))
    for i in range(5):
        var a: float = TAU*float(i)/5.0
        draw_circle(p+Vector2(cos(a),sin(a))*34.0*s,7.0*s,Color("39565b"))

func _brand(p: Vector2, s: float) -> void:
...
func _tree(p: Vector2, s: float) -> void:
    ...
    draw_circle(p+Vector2(-9.0,-2.0)*s,9.0*s,GREEN_2)
...
func _fountain(p: Vector2, s: float) -> void:
    _ellipse_shape(p,34.0*s,15.0*s,Color("8caeaa"),Color("507a78"))
    draw_circle(p,8.0*s,Color("d9e4db"))
    draw_line(p,p+Vector2(0.0,-18.0)*s,Color(0.64,0.84,0.82,0.65),2.0*s)

func _ellipse_shape(center: Vector2, rx: float, ry: float, fill: Color, edge: Color) -> void:
...
func _vehicle(p: Vector2, s: float, variant: int) -> void:
    ...
    _plate(p,44.0*s,22.0*s,6.0*s,c,c.darkened(0.25))
    draw_rect(Rect2(p+Vector2(-10.0,-11.0)*s,Vector2(20.0,7.0)*s),Color("315e66"),true)
    draw_circle(p+Vector2(-13.0,9.0)*s,4.0*s,Color("142126"))
    draw_circle(p+Vector2(13.0,9.0)*s,4.0*s,Color("142126"))

func _badge(pos: Vector2, title: String, value: String) -> void:
    var r := Rect2(pos,Vector2(196.0,52.0))
    draw_rect(r,Color(0.04,0.10,0.12,0.82),true)
    draw_rect(r,Color(0.44,0.57,0.56,0.38),false,1.0)
...
func _glow(center: Vector2, radius: float, tint: Color, strength: float) -> void:
    for i in range(5,0,-1):
        var radius_i: float = radius*float(i)/5.0
        var alpha: float = strength*(1.0-float(i)/6.0)
        draw_circle(center,radius_i,Color(tint.r,tint.g,tint.b,alpha))
