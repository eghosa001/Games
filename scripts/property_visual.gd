...
func _draw_property(origin: Vector2, property: Dictionary, stage: String) -> void:
    ...
    var wall := Color("555d63").lerp(Color("b8c4ca"), paint_amount).lerp(Color("cbd7dc"), furnish_amount * 0.18)
    ...
    for i in range(window_count):
        ...
        var light_threshold := 0.08 + furnish_amount * 0.72
        if stage == "Operational": light_threshold = 1.0
        var lit := float(i) / maxf(float(window_count - 1), 1.0) <= light_threshold
        var glass := Color("dcecf2").lerp(Color("f2d27a"), furnish_amount * 0.55) if lit else Color("252c31")
    ...
    if stage == "Operational":
        ...
        var operational_font := ThemeDB.fallback_font
...
func _draw_status_panel(origin: Vector2, property: Dictionary, stage: String) -> void:
    ...
    var state = get_node_or_null("/root/RenewGameState")
    var business_open := bool(state.get_value("businesses", "business_open", false)) if state != null else false
    var panel_height := 104.0 if not (stage == "Operational" and not business_open) else 174.0
    var panel := Rect2(origin.x - 150, origin.y + 112, 300, panel_height)
    ...
    if stage == "Operational" and not business_open:
        var purposes := _business_purposes()
