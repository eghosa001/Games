...
func settle_debt_day() -> Dictionary:
    ...
    for id in financing:
        ...
        var scheduled_enabled := int(instrument.get("payment", 0)) > 0
        ...
        # A zero payment is an explicit accrual-only state. Do not manufacture a
        # scheduled cash payment when callers have deliberately disabled one.
        # Normal instruments retain the amortization/maturity lifecycle below.
        var due := 0
        if scheduled_enabled and remaining_periods > 0:
    ...
    if scheduled_due > 0 and debt > 0:
        ...
        if cash >= payment:
            ...
            for id in financing:
                ...
                var actual_instrument_payment: int = interest_paid + principal_paid
                if actual_instrument_payment <= 0:
                    continue

                accrued = max(0.0, accrued - interest_paid)
                principal = max(0.0, principal - principal_paid)
                remaining_cash_payment -= actual_instrument_payment
                interest_paid_total += interest_paid
                principal_paid_total += principal_paid

                # A scheduled period is consumed only when cash was actually paid.
