...
func spend(amount: int, reason: String = "expense") -> Dictionary:
    if amount < 0 or cash < amount:
        return {"ok": false, "amount": 0, "reason": reason, "message": "Insufficient cash."}
    cash -= amount
...
func receive(amount: int, reason: String = "income") -> Dictionary:
    if amount < 0:
        return {"ok": false, "amount": 0, "reason": reason}
    cash += amount
...
func take_loan(amount: int) -> Dictionary:
    if amount <= 0:
        return {"ok": false, "message": "Invalid loan amount."}
    if debt > 0:
        return {"ok": false, "message": "An outstanding loan already exists."}
    debt = amount
    loan_payment = int(ceil(float(amount) / 20.0))
    cash += amount
...
func repay(amount: int) -> Dictionary:
    ...
    if cash < payment:
        return {"ok": false, "message": "Insufficient cash for repayment."}
    cash -= payment
    debt -= payment
    if debt == 0:
        loan_payment = 0
...
func settle_debt_day() -> Dictionary:
    ...
    var missed := false
    if debt > 0:
        interest = max(100, int(round(debt * 0.012)))
        cash -= interest
    ...
    if loan_payment > 0 and debt > 0:
        ...
        if cash >= payment:
            cash -= payment
            debt -= payment
            _record("scheduled_payment", payment, "scheduled loan payment")
        else:
            missed = true
    if debt == 0:
        loan_payment = 0
...
func restore_state(snapshot: Dictionary) -> void:
    ...
    cash = int(snapshot.get("cash", cash))
    debt = int(snapshot.get("debt", debt))
    loan_payment = int(snapshot.get("loan_payment", loan_payment))
    last_sales = int(snapshot.get("last_sales", 0))
    last_profit = int(snapshot.get("last_profit", 0))
    total_profit = int(snapshot.get("total_profit", 0))
