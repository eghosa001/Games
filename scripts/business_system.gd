...
func produce_goods()->void:
    ...
    var quality:=clamp(int(round(72.0+employee_factor*5.0+morale_multiplier*4.0+_technology_multiplier()*3.0+randi_range(-3,4))),30,100);var output:=max(1,cycles-int(floor(float(cycles)*0.08));production.system.finished_goods+=output;production.system.inventory["goods"]=production.system.finished_goods
