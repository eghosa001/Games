...
## Phase 20: the complete V1 industry model. These are static industry definitions;
## live player-owned facts remain in GameState.businesses.
const INDUSTRIES := {
    "furniture": {
        "id":"furniture", "name":"Furniture", "inputs":{"timber":1.0,"metal":0.5,"energy":2.0},
        "output":{"product":"furniture","units":1}, "workers":3, "capacity":6,
        "operating_cost":70, "base_price":220, "market_demand":80
    },
    "construction_materials": {
        "id":"construction_materials", "name":"Construction Materials", "inputs":{"timber":1.0,"stone":2.0,"energy":2.5},
        "output":{"product":"construction_materials","units":1}, "workers":4, "capacity":5,
        "operating_cost":95, "base_price":180, "market_demand":100
    },
    "consumer_electronics": {
        "id":"consumer_electronics", "name":"Consumer Electronics", "inputs":{"metal":1.0,"electronics_components":1.0,"energy":3.0},
        "output":{"product":"consumer_electronics","units":1}, "workers":5, "capacity":4,
        "operating_cost":130, "base_price":360, "market_demand":70
    }
}
...
func choose_business_purpose(index: int) -> void:
    ...
    state_adapter.set_value("businesses", "industry_id", str(purpose.get("industry_id", "")))
...
func create_business(purpose_id: String = "") -> void:
    ...
    var industry_id := str(purpose.get("industry_id", "")); var industry := get_industry(industry_id)
    if industry.is_empty(): state_adapter.message("Selected business has no valid V1 industry."); return
    var cash := int(state_adapter.get_value("economy", "cash", 25000))
    if cash < 3000: state_adapter.message("Need $3,000 working capital to launch the business."); return
    ...
    state_adapter.set_value("businesses", "business_purpose", str(purpose.get("id", "")))
    state_adapter.set_value("businesses", "industry_id", industry_id)
    ...
    state_adapter.log_message("BUSINESS CREATED: %s — %s industry — origin %s (%s)." % [purpose.get("name", "Business"), industry.get("name", "Industry"), property.get("name", "Property"), property_id])
...
func produce_goods() -> void:
    if not bool(state_adapter.get_value("businesses", "business_open", false)): state_adapter.message("Create the business first."); return
    var industry_id := str(state_adapter.get_value("businesses", "industry_id", "")); var industry := get_industry(industry_id)
    if industry.is_empty(): state_adapter.message("Business has no valid V1 industry."); return
    ...
    var capacity:=int(state_adapter.get_value("businesses", "capacity_level", 1)); var business_efficiency:=clamp(0.85+float(capacity)*0.10,0.85,1.50); var base_output:=max(1,employee_count+capacity-1)
    var output_factor:=employee_factor*business_efficiency*_technology_multiplier()*_property_condition_multiplier()*morale_multiplier; var cycles:=max(1,int(floor(float(base_output)*output_factor)))
    var cash:=int(state_adapter.get_value("economy","cash",25000)); var orders:=[]
    for resource in industry.inputs.keys():
        var need:=float(industry.inputs[resource])*cycles
        if supply_chain.stock(resource) < need: orders.append({"resource":resource,"amount":max(0.0,need-supply_chain.stock(resource))})
    if not orders.is_empty():
        var transport_level:=int(state_adapter.get_value("supply_chain","transport_level",1)); var delivery:=supply_chain.procure_bundle(orders,cash,transport_level)
        if not delivery["ok"]: state_adapter.message("Factory stopped: supply delivery failed (%s)." % str(delivery.get("reason","unknown"))); return
    ...
    var output:=max(1,cycles - int(floor(float(cycles) * 0.08))); production.system.finished_goods += output; production.system.inventory["goods"] = production.system.finished_goods
    production.system.last_run={"product":industry.output.product,"stage":"manufacturing","cycles":cycles,"output":output,"quality":quality,"inputs":industry.inputs,"workers":industry.workers,"capacity":industry.capacity,"base_price":industry.base_price,"operating_cost":industry.operating_cost,"market_demand":industry.market_demand}
    supply_chain.receive_furniture(output); state_adapter.set_value("production", "finished_goods", production.system.finished_goods)
    state_adapter.message("%s produced %d %s. Staff efficiency %.2fx." % [_business_name(),output,industry.output.product,output_factor])
    state_adapter.log_message("INDUSTRY: %s — %d units produced from %s." % [industry.name,output,_origin_property_name()])
