...
func get_business_purpose(purpose) -> Dictionary:
    ...
    for choice in choices:
        if str(choice.get("id", "")) == purpose_id: return choice.duplicate(true)
    return {}
...
func open_business() -> void:
    ...
    if existing_purpose.is_empty(): state_adapter.message("Choose what this restored property will become: press 4, 5 or 6."); return
...
func choose_business_purpose(index: int) -> void:
    ...
    if purpose.is_empty(): state_adapter.message("Invalid business purpose."); return
...
func create_business(purpose_id: String = "") -> void:
    ...
    if purpose.is_empty(): state_adapter.message("Choose a business purpose first: press 4, 5 or 6."); return
    var cost := 3000
    var cash: int = int(state_adapter.get_value("economy", "cash", 25000))
    if cash < cost: state_adapter.message("Need $3,000 working capital to launch the business."); return
    ...
    var spend := state_adapter.spend(cost, "business launch")
...
func upgrade_business() -> void:
    ...
    var level: int = int(state_adapter.get_value("businesses", "capacity_level", 1)); var cost: int = 4500 * level
    var cash: int = int(state_adapter.get_value("economy", "cash", 25000))
    if cash < cost: state_adapter.message("Capacity upgrade requires $%s." % state_adapter.money(cost)); return
    var spend := state_adapter.spend(cost, "business capacity upgrade")
    if not bool(spend.get("ok", false)): state_adapter.message(str(spend.get("message", "Capacity upgrade failed."))); return
    state_adapter.set_value("businesses", "capacity_level", level + 1); state_adapter.set_value("player", "reputation", int(state_adapter.get_value("player", "reputation", 0)) + 2)
...
func marketing_campaign() -> void:
    if not bool(state_adapter.get_value("businesses", "business_open", false)): state_adapter.message("Create the business first."); return
    var level: int = int(state_adapter.get_value("businesses", "marketing_level", 0)); var cost: int = 1800 + level * 700
    var cash: int = int(state_adapter.get_value("economy", "cash", 25000))
    if cash < cost: state_adapter.message("Marketing requires $%s." % state_adapter.money(cost)); return
    var spend := state_adapter.spend(cost, "marketing campaign")
    if not bool(spend.get("ok", false)): state_adapter.message(str(spend.get("message", "Marketing payment failed."))); return
    state_adapter.set_value("businesses", "marketing_level", level + 1); state_adapter.set_value("player", "reputation", int(state_adapter.get_value("player", "reputation", 0)) + 2)
...
# BusinessSystem owns this transaction. Callers do not need to understand the
# cross-ledger mutation sequence below. The transaction deliberately includes
# GameState, FinanceSystem, ProductionSystem, Economy and SupplyChain so a
# failure after any earlier mutation restores the complete production boundary.
func _production_transaction_participants() -> Array:
    return [
        ...
        {"name":"production","node":production},
        {"name":"economy","node":economy},
        {"name":"supply_chain","node":supply_chain},
    ]

func _capture_production_transaction() -> Dictionary:
    var snapshot: Dictionary = {"participants":{},"businesses":state_adapter.get_value("businesses","__transaction_missing__",{}).duplicate(true)}
    ...
    snapshot["economy_resources"] = economy.resources.duplicate(true)
    snapshot["economy_market_multipliers"] = economy.market_multipliers.duplicate(true)
    snapshot["economy_suppliers"] = economy.suppliers.duplicate(true)
...
func _restore_production_transaction(snapshot: Dictionary) -> void:
    ...
    if finance != null and participants.get("finance",null) is Dictionary: finance.restore_state(participants["finance"])
    if production != null and participants.get("production",null) is Dictionary: production.restore_state(participants["production"])
    economy.resources = snapshot.get("economy_resources",{}).duplicate(true)
    economy.market_multipliers = snapshot.get("economy_market_multipliers",{}).duplicate(true)
    economy.suppliers = snapshot.get("economy_suppliers",{}).duplicate(true)
...
func _production_failure(message: String) -> Dictionary:
    state_adapter.message(message)
    return {"ok":false,"message":message}

func produce_goods() -> Dictionary:
    var transaction := _capture_production_transaction()
    if not bool(transaction.get("ok",false)):
        return _production_failure(str(transaction.get("message","Production transaction could not start.")))
    ...
    state_adapter.message(str(result.get("message","Production failed and was rolled back.")))
    return result
...
func _produce_goods_impl() -> Dictionary:
    ...
    if config.is_empty(): return _production_failure("Unknown V1 industry: %s." % industry_id)
    var employee_count: int = 3; var employee_factor: float = 1.0; var morale_multiplier: float = 1.0
    if employee_system != null:
        employee_count = employee_system.get_active_employee_count(); employee_factor = employee_system.get_productivity_multiplier("factory_001"); morale_multiplier = employee_system.get_morale_multiplier()
    var capacity: int = int(state_adapter.get_value("businesses", "capacity_level", 1)); var business_efficiency: float = clamp(0.85 + float(capacity) * 0.10, 0.85, 1.50)
    var base_output: int = max(1, employee_count + capacity - 1); var output_factor: float = employee_factor * business_efficiency * _technology_multiplier() * _property_condition_multiplier() * morale_multiplier
    var cycles: int = max(1, int(floor(float(base_output) * output_factor))); var inputs: Dictionary = config.get("inputs", {}).duplicate(true)
    var cash: int = int(state_adapter.get_value("economy", "cash", 25000)); var orders: Array = []
    for resource in inputs:
        var input_name: String = str(resource)
        if input_name == "metal": continue
        var needed: float = float(inputs[resource]) * float(cycles); var current_stock: float = supply_chain.stock(input_name)
        if current_stock < needed: orders.append({"resource":input_name,"amount":needed-current_stock})
    ...
    if not bool(input_result.get("ok", false)): return _production_failure("%s stopped: warehouse inputs are too low." % _business_name())
    ...
    var receipt := supply_chain.receive_product(product,output)
    if not bool(receipt.get("ok",false)): return _production_failure("%s stopped: finished-goods receipt failed (%s)." % [_business_name(), str(receipt.get("reason","unknown"))])
    var production_spend := state_adapter.spend(operating_cost, "production operating cost")
    if not bool(production_spend.get("ok", false)): return _production_failure("%s stopped: production operating cost could not be paid." % _business_name())
    state_adapter.set_value("production","finished_goods",production.finished_goods)
    var success_message := "%s produced %d %s at quality %d. Staff efficiency %.2fx." % [_business_name(),output,product.replace("_"," "),quality,output_factor]
    state_adapter.message(success_message); state_adapter.log_message("FACTORY: %s (from %s) -> Warehouse -> Customer; %d %s produced." % [_business_name(),_origin_property_name(),output,product])
    return {"ok":true,"message":success_message,"product":product,"output":output,"quality":quality}

func _prepare_derived_inputs(inputs: Dictionary, cycles: int) -> bool:
    var metal_needed: float = float(inputs.get("metal", 0.0)) * float(cycles)
    if metal_needed <= supply_chain.stock("metal"): return true
    var missing: float = metal_needed - supply_chain.stock("metal"); var metal_cycles: int = int(ceil(missing / 1.0)); var result: Dictionary = supply_chain.process_iron_to_metal(metal_cycles)
...
func capture_state() -> Dictionary:
    return {"system_version":3,"businesses":state_adapter.get_value("businesses","businesses",{}).duplicate(true)}
func restore_state(snapshot:Dictionary)->void:
    if snapshot.is_empty(): return
    var businesses=snapshot.get("businesses",{})
    if businesses is Dictionary: state_adapter.set_value("businesses","businesses",businesses.duplicate(true))