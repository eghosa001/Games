...
func upgrade_business() -> void:
    ...
    var level: int = int(state_adapter.get_value("businesses", "capacity_level", 1)); var cost: int = 4500 * level; var cash: int = int(state_adapter.get_value("economy", "cash", 35000)); if cash < cost: state_adapter.message("Capacity upgrade requires $%s." % state_adapter.money(cost)); return
...
func marketing_campaign() -> void:
    if not bool(state_adapter.get_value("businesses", "business_open", false)): state_adapter.message("Create the business first."); return
    var level: int = int(state_adapter.get_value("businesses", "marketing_level", 0)); var cost: int = 1800 + level * 700; var cash: int = int(state_adapter.get_value("economy", "cash", 35000)); if cash < cost: state_adapter.message("Marketing requires $%s." % state_adapter.money(cost)); return
