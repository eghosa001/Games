...
func create_business(purpose_id: String = "") -> void:
    ...
    var cost := 3000; var cash: int = int(state_adapter.get_value("economy", "cash", 25000))
    if cash < cost: state_adapter.message("Need $3,000 working capital to launch the business."); return
    var property: Dictionary = _selected_property(); var property_id: String = str(property.get("id", "")); var business_id: String = "%s_%s" % [purpose.get("id", "business"), property_id]
...
func _origin_property_type() -> String:
    var stored: String = str(state_adapter.get_value("businesses", "origin_property_type", "")); if not stored.is_empty(): return stored
    return str(_selected_property().get("type", ""))
func _origin_property_name() -> String:
    var stored: String = str(state_adapter.get_value("businesses", "origin_property_name", "")); if not stored.is_empty(): return stored
...
func upgrade_business() -> void:
    ...
    var level: int = int(state_adapter.get_value("businesses", "capacity_level", 1)); var cost: int = 4500 * level; var cash: int = int(state_adapter.get_value("economy", "cash", 25000))
...
func marketing_campaign() -> void:
    if not bool(state_adapter.get_value("businesses", "business_open", false)): state_adapter.message("Create the business first."); return
    var level: int = int(state_adapter.get_value("businesses", "marketing_level", 0)); var cost: int = 1800 + level * 700; var cash: int = int(state_adapter.get_value("economy", "cash", 25000))
...
func change_price() -> void:
    if not bool(state_adapter.get_value("businesses", "business_open", false)): state_adapter.message("Create the business first."); return
    var price: int = int(state_adapter.get_value("businesses", "player_price", 110)) + 10; if price > 160: price = 80
...
func _technology_multiplier() -> float:
    ...
    var level: int = 0; for key in technology.keys(): level += int(technology[key])
...
func produce_goods() -> void:
    if not bool(state_adapter.get_value("businesses", "business_open", false)): state_adapter.message("Create the business first."); return
    if production == null: state_adapter.message("ProductionSystem is unavailable."); return
    ...
    var capacity: int = int(state_adapter.get_value("businesses", "capacity_level", 1)); var business_efficiency: float = clamp(0.85 + float(capacity) * 0.10, 0.85, 1.50); var base_output: int = max(1, employee_count + capacity - 1); var output_factor: float = employee_factor * business_efficiency * _technology_multiplier() * _property_condition_multiplier() * morale_multiplier; var cycles: int = max(1, int(floor(float(base_output) * output_factor)); var inputs: Dictionary = config.get("inputs", {}).duplicate(true)
    ...
    if not orders.is_empty():
        ...
        if not bool(delivery.get("ok", false)): state_adapter.message("%s stopped: supply delivery failed (%s)." % [_business_name(), str(delivery.get("reason", "unknown"))]); return
        var delivery_cost := int(delivery["cost"]); var spend := state_adapter.spend(delivery_cost, "supply delivery")
        if not bool(spend.get("ok", false)):
            ...
            state_adapter.message("%s stopped: supply payment failed." % _business_name()); return
        state_adapter.log_message("SUPPLY: market resources delivered to %s." % _business_name())
    if not _prepare_derived_inputs(inputs, cycles): state_adapter.message("%s stopped: intermediate inputs are unavailable." % _business_name()); return
    var input_result: Dictionary = supply_chain.consume_inputs(inputs, cycles); if not bool(input_result.get("ok", false)): state_adapter.message("%s stopped: warehouse inputs are too low." % _business_name()); return
    ...
    # Sales and contract settlement operate on the persistent production total. Refresh it before adding new output so sold goods are never resurrected.
    ...
    supply_chain.receive_product(product,output); state_adapter.set_value("production","finished_goods",production.finished_goods); state_adapter.message("%s produced %d %s at quality %d. Staff efficiency %.2fx." % [_business_name(),output,product.replace("_"," "),quality,output_factor]); state_adapter.log_message("FACTORY: %s (from %s) -> Warehouse -> Customer; %d %s produced." % [_business_name(),_origin_property_name(),output,product])
...
func restore_state(snapshot:Dictionary)->void:
    ...
    var businesses=snapshot.get("businesses",{}); if businesses is Dictionary: state_adapter.set_value("businesses","businesses",businesses.duplicate(true))
