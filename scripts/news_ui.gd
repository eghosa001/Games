...
func _refresh(force: bool = false) -> void:
    ...
    for story in stories:
        ...
        if section != current_section: current_section = section; _add_section(section)
        _add_story(story)
    _layout(); scroll.set_deferred("scroll_vertical", scroll_value)

func _add_section(section: String) -> void:
    var label := Label.new(); label.text = section.to_upper(); label.add_theme_font_size_override("font_size", 11); label.add_theme_color_override("font_color", ACCENT); content.add_child(label)
func _add_story(story: Dictionary) -> void:
...
func _layout() -> void:
    ...
    var width := maxf(304.0, size.x - 16.0) if narrow else minf(700.0, size.x - 36.0); var height := maxf(400.0, size.y - 86.0) if narrow else minf(680.0, size.y - 100.0)
