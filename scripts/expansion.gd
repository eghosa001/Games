...
func _normalize_all() -> void:
    for p in properties:
        ...
        p["cost"] = int(p.get("cost", p["restoration_cost"]))
        p["income"] = int(p.get("income", 0))
        p["level"] = max(1, int(p.get("level", 1)))
        ...
        p["unlock_rep"] = int(p.get("unlock_rep", 10))
    for site in resource_sites:
        site["cost"] = int(site.get("cost", 15000))
        site["level"] = max(1, int(site.get("level", 1)))
        site["output"] = max(0, int(site.get("output", 0)))
        site["risk"] = clamp(int(site.get("risk", 0)), 0, 100)
        site["owned"] = bool(site.get("owned", false))
        site["stock"] = max(0, int(site.get("stock", 0)))

func unlock_from_reputation(rep: int) -> void:
    _normalize_all()
    for p in properties:
        p["unlocked"] = int(rep) >= int(p["unlock_rep"])
...
func buy(index: int, cash_available: int) -> Dictionary:
    return restore_property(index, cash_available)

func upgrade(index: int, cash_available: int) -> Dictionary:
    return upgrade_property(index, cash_available)
...
func operate_day() -> Dictionary:
    ...
    for p in properties:
        if bool(p["owned"]) and bool(p.get("active", true)):
            ...
            revenue += int(p["income"]) * level
            expense += int(p["income"]) / 4
    for site in resource_sites:
        if bool(site["owned"]):
            revenue += int(site["output"]) * int(site["level"]) * 100
...
func upgrade_property(index: int, cash_available: int) -> Dictionary:
    _normalize_all()
    if index < 0 or index >= properties.size() or not bool(properties[index]["owned"]):
...
func generate_resource(index: int) -> Dictionary:
    _normalize_all()
    if index < 0 or index >= resource_sites.size() or not bool(resource_sites[index]["owned"]):
...
func supply_business(index: int, resource: String, amount: int) -> Dictionary:
    ...
    var available: int = int(source["stock"])
