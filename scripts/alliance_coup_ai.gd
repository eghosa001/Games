...
func _consider_alliance(system: Node, control: Node, alliance: Dictionary, npcs: Array, day: int) -> void:
    ...
    var chairman: Variant = _find(npcs, chairman_id)
    var chairman_rel: Variant = int(chairman.get("relationship", 0))
    var chairman_contribution: Variant = int(members.get(chairman_id, {}).get("contributed", 0))
    var challenger_id: Variant = ""
    var best_score: Variant = -999999.0
    var support: Variant = 0
    for member_id in members.keys():
        var id: Variant = str(member_id)
        if id == chairman_id: continue
        var npc: Variant = _find(npcs, id)
        if npc.is_empty(): continue
        var contribution: Variant = int(members[id].get("contributed", 0))
        var relationship: Variant = int(npc.get("relationship", 0))
        var role: Variant = str(members[id].get("role", ""))
        if role != "director": continue
        if contribution <= max(1, chairman_contribution) * 1.25: continue
        var score: Variant = float(contribution) + float(relationship) * 500.0
        if score > best_score:
            best_score = score
            challenger_id = id
    if challenger_id.is_empty(): return
    var challenger: Variant = _find(npcs, challenger_id)
    for member_id in members.keys():
        var voter_id: Variant = str(member_id)
        if voter_id == challenger_id:
            support += 1
            continue
        var voter: Variant = _find(npcs, voter_id)
        if voter.is_empty(): continue
        var relationship: Variant = int(voter.get("relationship", 0))
        if relationship >= 15: support += 1
        elif relationship >= 5 and chairman_rel < 0: support += 1
    var support_ratio: Variant = float(support) / float(members.size())
