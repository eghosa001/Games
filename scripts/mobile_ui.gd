...
func _layout_responsive() -> void:
    ...
    if narrow:
        ...
        goal_label.position = Vector2(12, 354)
        goal_label.size = Vector2(w - 24.0, 58)
    else:
        ...
        goal_label.position = Vector2(24, 348)
        goal_label.size = Vector2(minf(900.0, w - 48.0), 48)
...
func _run_action(label: String, callback: Callable) -> void:
    ...
    _show_feedback(message); _pulse_feedback(); _queue_refresh()
    _detect_milestones()
...
func _detect_milestones() -> void:
    ...
    for id in current:
        if bool(current[id]) and not bool(observed_milestones.get(id, false)):
            ...
            _show_feedback("★ MILESTONE UNLOCKED\n" + String(celebrations[id]))
            _pulse_feedback()
