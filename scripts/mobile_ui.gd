...
func _build_ui() -> void:
    ...
    var bottom := ColorRect.new(); bottom.color = Color("101820"); bottom.set_anchors_preset(Control.PRESET_BOTTOM_WIDE); bottom.position.y = -138; bottom.size.y = 138; bottom.mouse_filter = Control.MOUSE_FILTER_IGNORE; root.add_child(bottom)
    action_scroll = ScrollContainer.new(); action_scroll.position = Vector2(18, -132); action_scroll.set_anchors_preset(Control.PRESET_BOTTOM_LEFT); action_scroll.size = Vector2(1240, 126); action_scroll.horizontal_scroll_mode = ScrollContainer.SCROLL_MODE_DISABLED; action_scroll.vertical_scroll_mode = ScrollContainer.SCROLL_MODE_AUTO; action_scroll.focus_mode = Control.FOCUS_NONE; action_scroll.mouse_filter = Control.MOUSE_FILTER_STOP; root.add_child(action_scroll)
    actions = GridContainer.new(); actions.columns = 5; actions.custom_minimum_size = Vector2(1200, 0); actions.add_theme_constant_override("h_separation", 10); actions.add_theme_constant_override("v_separation", 8); actions.mouse_filter = Control.MOUSE_FILTER_IGNORE; action_scroll.add_child(actions)
