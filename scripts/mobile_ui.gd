...
func _initialize_after_parent_ready() -> void:
    ...
    _seed_milestones()
    if root != null and not root.resized.is_connected(_layout_responsive):
...
func _layout_responsive() -> void:
    ...
    var tab_width: float = maxf(70.0, (w - 48.0) / 4.0)
    ...
    if narrow:
        ...
        feedback_panel.position = Vector2(12, 252); feedback_panel.size = Vector2(w - 24.0, 96); feedback_label.size = Vector2(w - 56.0, 76)
        goal_label.position = Vector2(12, 354); goal_label.size = Vector2(w - 24.0, 58)
    else:
        ...
        feedback_panel.position = Vector2(24, 245); feedback_panel.size = Vector2(minf(720.0, w - 48.0), 92); feedback_label.size = Vector2(feedback_panel.size.x - 32.0, 72)
        goal_label.position = Vector2(24, 348); goal_label.size = Vector2(minf(900.0, w - 48.0), 48)
    if bottom_bar != null:
        bottom_bar.position.y = h - 150.0
...
func _detect_milestones() -> void:
    ...
    for id: String in current:
        if bool(current[id]) and not bool(observed_milestones.get(id, false)):
            observed_milestones[id] = true; _show_feedback("★ MILESTONE UNLOCKED\n" + String(celebrations[id])); _pulse_feedback()
