...
func update_company(company_id: String, metrics: Dictionary) -> Dictionary:
    var entry := register_company(company_id)
...
func process_day(day: int) -> void:
    ...
    _discover_companies()
    var snapshot := _build_snapshot(day)
...
func _build_snapshot(day: int) -> Dictionary:
    var snapshot := {"day": day, "timestamp": Time.get_unix_time_from_system(), "rankings": {}}
    ...
    if rivals is Array:
        for rival in rivals:
            if rival is Dictionary:
                var id := str(rival.get("id", rival.get("name", "rival")))
                var region := str(rival.get("region", "global"))
                register_company(id, str(rival.get("name", id)), region)
                update_company(id, _derive_metrics(rival))
    var player_id := "founder"
...
func _derive_player_metrics(main) -> Dictionary:
    ...
    var cash := float(main.get("cash", 0.0))
    var reputation := float(main.get("reputation", 0.0))
    var revenue := float(main.get("revenue", 0.0))
    var profit := float(main.get("profit", 0.0))
    var assets := cash
    var valuation := cash
    if finance != null:
    ...
    var infra_score := float(infrastructure.total_capacity("")) if infrastructure != null else 0.0
    var employee_count := float(employees.active_count()) if employees != null else float(main.get("employees", 0))
    var alliance_score := 0.0
    if alliance != null:
