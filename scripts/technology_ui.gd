...
func _refresh() -> void:
    ...
    for item in tech.get_technologies():
        ...
        var status := "RESEARCHED" if researched else ("READY" if bool(can.get("ok", false)) else "LOCKED")
        var card := Panel.new(); card.custom_minimum_size = Vector2(0, 112 if get_viewport().get_visible_rect().size.x < 430.0 else 96); card.add_theme_stylebox_override("panel", _style(SURFACE_2)); list.add_child(card)
        ...
        var meta := Label.new(); meta.text = "Tier %d  •  $%d  •  %d RP  •  %d day%s  •  %s%s" % [int(item.tier), int(item.cost_money), int(item.cost_points), int(item.time_days), "" if int(item.time_days) == 1 else "s", status, ("  •  " + reason) if reason != "" else ""]; meta.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART; meta.add_theme_font_size_override("font_size",10); meta.add_theme_color_override("font_color", ACCENT if bool(can.get("ok", false)) else MUTED); card.add_child(meta)
        var button := Button.new(); button.text = "DONE" if researched else "RESEARCH"; button.disabled = researched or not bool(can.get("ok", false)); button.custom_minimum_size = Vector2(96,46); button.focus_mode = Control.FOCUS_NONE; button.pressed.connect(_research.bind(id)); card.add_child(button)
    _layout()
...
func _layout() -> void:
    ...
    for card in list.get_children():
        if card is Panel:
            ...
            var card_h := 112.0 if phone else 96.0
            card.custom_minimum_size = Vector2(0, card_h)
            if phone:
                if name != null: name.position = Vector2(12,8); name.size = Vector2(w-48,40)
                if meta != null: meta.position = Vector2(12,48); meta.size = Vector2(w-48,52)
                if button != null: button.position = Vector2(w-124, card_h-54); button.size = Vector2(108,46)
            else:
                if name != null: name.position = Vector2(12,8); name.size = Vector2(maxf(110,w-144),24)
                if meta != null: meta.position = Vector2(12,36); meta.size = Vector2(maxf(110,w-144),50)
                if button != null: button.position = Vector2(w-124,25); button.size = Vector2(108,46)
