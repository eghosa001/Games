...
func _layout() -> void:
    ...
    for card in list.get_children():
        ...
        var card_h := 142.0 if phone else 102.0; card.custom_minimum_size = Vector2(0, card_h)
        if phone:
            if name != null: name.position = Vector2(12, 8); name.size = Vector2(w - 150, 36)
            if meta != null: meta.position = Vector2(12, 44); meta.size = Vector2(w - 150, card_h - 50)
