...
func _capture_domain_systems()->void:
    var root:=get_tree().root;var finance=root.get_node_or_null("RenewFinanceSystem");if finance!=null and finance.has_method("capture_state"):domains["finance"]["system"]=finance.capture_state()
...
func _restore_domain_systems()->void:
    var root:=get_tree().root;var finance=root.get_node_or_null("RenewFinanceSystem");if finance!=null and finance.has_method("restore_state") and domains["finance"].get("system",{}) is Dictionary:finance.restore_state(domains["finance"]["system"])
