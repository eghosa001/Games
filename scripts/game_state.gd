...
const DOMAIN_KEYS := {
    "player": ["day", "reputation"], "company": ["message", "log_lines"],
    ...
    "technology": ["technology", "research_points"], "events": ["history", "active", "modifiers"], "progression": ["milestones", "unlocks", "xp", "level"],
    "history": ["history_system"], "news": ["news_system"], "analytics": ["simulation_system"]
}

const DEFAULT_DOMAINS := {
    "player": {"day": 1, "reputation": 0}, "company": {"message": "Inspect the abandoned warehouse. Your empire starts here.", "log_lines": []},
    ...
    "contracts": {"contract_days": 0, "contract_bonus": 0, "contract_system": {}, "system": {}}, "competitors": {"rivals": [], "selected_rival": 0, "relationship": 15}, "ownership": {"acquisition_count": 0}, "finance": {"debt": 0, "loan_payment": 0, "system": {}}, "alliances": {"alliances": {}}, "diplomacy": {"diplomacy": {}},
    ...
    "technology": {"technology": {}, "research_points": 20}, "events": {"history": [], "active": {}, "modifiers": {}}, "progression": {"milestones": [], "unlocks": [], "xp": 0, "level": 1},
    "history": {"history_system": {}}, "news": {"news_system": {}}, "analytics": {"simulation_system": {}}
}
...
func restore(snapshot: Dictionary) -> Dictionary:
    ...
    var flat: Dictionary = snapshot["legacy"].duplicate(true) if snapshot.has("legacy") and snapshot["legacy"] is Dictionary else _domains_to_flat(snapshot)
    _apply_flat_to_domains(flat); _ensure_defaults(); _restore_domain_systems(); data = {"state_version": STATE_VERSION, "domains": domains.duplicate(true), "legacy": _domains_to_flat({"domains": domains})}; return flat.duplicate(true)
...
func set_value(domain: String, key: String, value) -> void:
    if not DOMAINS.has(domain): push_warning("GameState: rejected unknown domain '%s'" % domain); return
    if not DOMAIN_KEYS.get(domain, []).has(key): push_warning("GameState: '%s' does not belong to domain '%s'" % [key, domain]); return
...
func _apply_flat_to_domains(flat: Dictionary) -> void:
    ...
    for key in ["events", "active", "modifiers", "history"]: _migrate_key("events", flat, key)
