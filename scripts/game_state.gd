...
func new_game() -> Dictionary:
    ...
    _ensure_property_record(CORE_PROPERTY_ID)
    dirty = true
...
func capture(core: Dictionary) -> Dictionary:
    ...
    snapshot["schema_version"] = STATE_VERSION; snapshot["state_version"] = STATE_VERSION
...
func sync_property_runtime(core: Dictionary, property_id: String = CORE_PROPERTY_ID) -> void:
    ...
    var mapping := {"owned":"owned", "inspected":"inspected", "restoration":"restoration", "stage":"stage"}
    for key in mapping.keys():
        if core.has(mapping[key]): record[key] = core[mapping[key]]
...
func restore_property_runtime(core: Dictionary, property_id: String = CORE_PROPERTY_ID) -> void:
    ...
    core["owned"] = bool(record.get("owned", false))
    core["inspected"] = bool(record.get("inspected", false))
    core["restoration"] = int(record.get("restoration", 0))
    core["stage"] = str(record.get("stage", "Neglected"))
