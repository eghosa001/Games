...
func set_domain(domain: String, value: Dictionary) -> void:
    if not DOMAINS.has(domain): push_warning("GameState: rejected unknown domain '%s'" % domain); return
...
func set_value(domain: String, key: String, value) -> void:
    ...
    if not allowed.has(key): push_warning("GameState: '%s' does not belong to domain '%s'" % [key, domain]); return
...
func get_flat_state() -> Dictionary:
    _ensure_defaults(); return _domains_to_flat({"domains": domains})
func clear() -> void:
    data.clear(); domains.clear(); _ensure_domains(); _ensure_defaults()
...
func _migrate_key(domain: String, flat: Dictionary, key: String) -> void:
    if flat.has(key):
        var bucket: Dictionary = domains.get(domain, {}).duplicate(true); bucket[key] = flat[key]; domains[domain] = bucket
    ...
    for domain in incoming.keys():
        ...
        if bucket is Dictionary:
            for key in bucket.keys():
                if not _is_canonical_key(domain, str(key)): continue
...
func _capture_domain_systems() -> void:
    ...
    if finance != null and finance.has_method("capture_state"): domains["finance"]["system"] = finance.capture_state()
    var production = root.get_node_or_null("RenewProductionSystem")
    if production != null and production.has_method("capture_state"): domains["production"]["system"] = production.capture_state()
    var simulation = root.get_node_or_null("RenewSimulationSystem")
    if simulation != null and simulation.has_method("capture_state"): domains["analytics"]["simulation_system"] = simulation.capture_state()
    var contracts = root.get_node_or_null("RenewContractSystem")
    if contracts != null and contracts.has_method("capture_state"): domains["contracts"]["system"] = contracts.capture_state()

func _restore_domain_systems() -> void:
    var root := get_tree().root
    var finance = root.get_node_or_null("RenewFinanceSystem")
    if finance != null and finance.has_method("restore_state") and domains["finance"].get("system", {}) is Dictionary: finance.restore_state(domains["finance"]["system"])
    var production = root.get_node_or_null("RenewProductionSystem")
    if production != null and production.has_method("restore_state") and domains["production"].get("system", {}) is Dictionary: production.restore_state(domains["production"]["system"])
    var simulation = root.get_node_or_null("RenewSimulationSystem")
    if simulation != null and simulation.has_method("restore_state") and domains["analytics"].get("simulation_system", {}) is Dictionary: simulation.restore_state(domains["analytics"]["simulation_system"])
    var contracts = root.get_node_or_null("RenewContractSystem")
    if contracts != null and contracts.has_method("restore_state") and domains["contracts"].get("system", {}) is Dictionary: contracts.restore_state(domains["contracts"]["system"])
