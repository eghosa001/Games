...
func _build_ui() -> void:
    ...
    contract_scroll = ScrollContainer.new(); contract_scroll.horizontal_scroll_mode = ScrollContainer.SCROLL_MODE_DISABLED; panel.add_child(contract_scroll)
...
func _refresh() -> void:
    ...
    if active.is_empty(): detail_label.text = "Sign a customer contract to turn demand into scheduled production and delivery."; _layout(); return
...
func _add_contract_row(contract: Dictionary) -> void:
    ...
    var button := Button.new(); button.text = "%s  •  %s\n%d / %d  •  %d%%  •  %s" % [customer, product, delivered, quantity, roundi(progress * 100.0), status[0]]; button.alignment = HORIZONTAL_ALIGNMENT_LEFT; button.custom_minimum_size = Vector2(0, 54); button.focus_mode = Control.FOCUS_NONE
    button.add_theme_stylebox_override("normal", _button_style(SURFACE_2)); button.add_theme_stylebox_override("hover", _button_style(Color("17343d"))); button.add_theme_color_override("font_color", TEXT); button.pressed.connect(_select_contract.bind(id)); contract_list.add_child(button)
...
func _find_contract(contracts: Array, id: String) -> Dictionary:
    ...
    return {}
func _select_contract(id: String) -> void: selected_contract_id = id; _refresh()
...
func _layout() -> void:
    if panel == null: return
    var size := get_viewport().get_visible_rect().size; var narrow := size.x < 760.0; var width := maxf(304.0, size.x - 16.0) if narrow else minf(520.0, size.x - 36.0); var height := maxf(400.0, size.y - 92.0) if narrow else minf(650.0, size.y - 120.0)
    ...
    cancel_button.position = Vector2(12, height - 58.0); cancel_button.size = Vector2(width - 24.0, 46); detail_panel.position = Vector2(12, height - (142.0 if narrow else 158.0) - 70.0); detail_panel.size = Vector2(width - 24.0, 142.0 if narrow else 158.0); detail_scroll.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT); detail_label.custom_minimum_size = Vector2(width - 48.0, 0)
    contract_scroll.position = Vector2(12, 96); contract_scroll.size = Vector2(width - 24.0, maxf(100.0, detail_panel.position.y - 106.0)); contract_list.custom_minimum_size.x = width - 24.0; empty_label.position = Vector2(18, 120); empty_label.size = Vector2(width - 36.0, 70)
