...
func _add_contract_row(contract: Dictionary) -> void:
    ...
    button.text = "%s  •  %s\n%d / %d  •  %d%%  •  %s" % [customer, product, delivered, quantity, roundi(progress * 100.0), status[0]]
    ...
    if id == selected_contract_id: button.add_theme_color_override("font_pressed_color", ACCENT)
...
func _delivery_status(delivered: int, expected: float, days_left: int, quantity: int) -> Array:
    if days_left <= 0:
        return ["ON TRACK • COMPLETE", STATUS_GREEN] if delivered >= quantity else ["OVERDUE • ACTION NEEDED", STATUS_RED]
...
func _show_detail(contract: Dictionary) -> void:
    ...
    var fulfillment_ratio := float(delivered) / float(quantity)
...
func _layout() -> void:
    ...
    var width := maxf(304.0, size.x - 16.0) if narrow else 520.0
    var height := maxf(380.0, size.y - 92.0) if narrow else minf(650.0, size.y - 120.0)
    panel.position = Vector2(8, 70) if narrow else Vector2(maxf(18.0, size.x - width - 18.0), 90)
    panel.size = Vector2(width, height)
    title_label.position = Vector2(14, 10); title_label.size = Vector2(width - 100.0, 30)
    close_button.position = Vector2(width - 84.0, 7); close_button.size = Vector2(72, 44)
    summary_label.position = Vector2(14, 46); summary_label.size = Vector2(width - 28.0, 44)
    var button_h := 46.0
    cancel_button.position = Vector2(12, height - 58.0); cancel_button.size = Vector2(width - 24.0, button_h)
    detail_panel.position = Vector2(12, height - (142.0 if narrow else 158.0) - 70.0)
    detail_panel.size = Vector2(width - 24.0, 142.0 if narrow else 158.0)
    ...
    contract_scroll.position = Vector2(12, 96); contract_scroll.size = Vector2(width - 24.0, maxf(100.0, detail_panel.position.y - 106.0))
    contract_list.custom_minimum_size.x = width - 24.0
    empty_label.position = Vector2(18, 120); empty_label.size = Vector2(width - 36.0, 70)
