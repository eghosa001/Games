...
func _build_ui() -> void:
    ...
    title_label = Label.new(); title_label.text = "CUSTOMER CONTRACTS"; title_label.add_theme_font_size_override("font_size", 20); title_label.add_theme_color_override("font_color", TEXT); panel.add_child(title_label)
...
func _refresh(force: bool = false) -> void:
    ...
    for contract in active:
        signature += ":%s:%d:%d:%d:%d:%d" % [str(contract.get("id", "")), int(contract.get("quantity_delivered", 0)), int(contract.get("quantity_due", 0)), int(contract.get("days_elapsed", 0)), int(contract.get("penalties_paid", 0)), int(contract.get("price", 0))]
    ...
    if active.is_empty():
        selected_contract_id = ""; detail_label.text = "Sign a customer contract to turn demand into scheduled production and delivery."; _layout(); return
...
func _layout() -> void:
    ...
    panel.position = Vector2(8, 70) if narrow else Vector2(maxf(18.0, size.x - width - 18.0), 90); panel.size = Vector2(width, height); title_label.position = Vector2(14, 10); title_label.size = Vector2(width - 100.0, 30); close_button.position = Vector2(width - 90.0, 7); close_button.size = Vector2(80, 46); summary_label.position = Vector2(14, 46); summary_label.size = Vector2(width - 28.0, 44)
    var detail_height := 166.0 if phone else 158.0
    detail_panel.position = Vector2(12, height - detail_height - 70.0); detail_panel.size = Vector2(width - 24.0, detail_height); detail_scroll.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT); detail_label.position = Vector2(10, 8); detail_label.size = Vector2(width - 44.0, detail_height - 16.0); detail_label.custom_minimum_size = Vector2(width - 48.0, 0)
