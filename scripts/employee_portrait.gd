...
func _draw() -> void:
    var w := size.x
    var h := size.y
    var skin_options := [Color("C98F6B"), Color("B97858"), Color("E0A27B"), Color("8E5A43")]
    var hair_options := [Color("171717"), Color("3A2922"), Color("5B4438"), Color("2B3542")]
    var skin := skin_options[seed % skin_options.size()]
    var hair := hair_options[(seed / 7) % hair_options.size()]
    var jacket := accent.darkened(0.48)
    ...
    draw_circle(Vector2(w * 0.42, 42), 2.2, Color("18202C"))
    draw_circle(Vector2(w * 0.58, 42), 2.2, Color("18202C"))
