...
func record_market_share(player_state: Dictionary, day: int) -> Array[String]:
    ...
    for rival in rivals.rivals:
        var price := max(1, int(rival.get("price", 110)))
        var production_capacity := max(0, int(rival.get("production", 0)))
        var reputation := clamp(float(rival.get("reputation", 50)) / 100.0, 0.0, 1.0)
        var price_factor := clamp(1.0 + (float(player_price - price) / float(player_price)) * 0.55, 0.55, 1.45)
        ...
        var daily_sales := int(round(float(production_capacity) * price_factor * strategy_factor))
        var inventory_cap := max(production_capacity, int(rival.get("inventory", production_capacity)))
        daily_sales = min(daily_sales, max(0, inventory_cap))
        rival["last_sales"] = daily_sales
        rival["total_sales"] = int(rival.get("total_sales", 0)) + daily_sales
        competitor_sales_total += daily_sales
        competitor_rows.append({"id": str(rival.get("id", "")), "name": str(rival.get("name", "Rival")), "sales": daily_sales})

    var total_market_sales := player_sales + competitor_sales_total
    current_player_sales = player_sales
    current_competitor_sales = competitor_sales_total
    current_market_share = float(player_sales) / float(max(1, total_market_sales))

    if total_market_sales > 0:
        for rival in rivals.rivals:
            var rival_sales := int(rival.get("last_sales", 0))
...
func get_market_share() -> float:
    return current_market_share

func get_market_share_history() -> Dictionary:
    return market_share_history.duplicate(true)

func get_rankings() -> Array[Dictionary]:
    var ranking: Array[Dictionary] = []
    ranking.append({"id": "player", "name": "Your Company", "market_share": current_market_share, "sales": current_player_sales})
    if rivals != null:
        for rival in rivals.rivals:
...
func restore_state(snapshot: Dictionary) -> void:
    ...
    market_share_history = snapshot.get("market_share_history", {}).duplicate(true)
    current_market_share = float(snapshot.get("current_market_share", 0.0))
    current_player_sales = int(snapshot.get("current_player_sales", 0))
    current_competitor_sales = int(snapshot.get("current_competitor_sales", 0))