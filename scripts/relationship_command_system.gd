...
func _selected_rival() -> Dictionary:
    ...
    if index < 0:
        return {}
    return rivals.rivals[index]

func select_rival(index: int) -> void:
    if index < 0 or index >= rivals.rivals.size():
        state_adapter.message("No such rival is available.")
        return
    state_adapter.set_value("competitors", "selected_rival", index)
    state_adapter.set_value("competitors", "relationship", int(rivals.rivals[index].get("relationship", 0)))
    state_adapter.message("Selected %s." % rivals.rivals[index].get("name", "rival"))

func improve_alliance() -> void:
    var selected := _selected_index()
    if selected < 0:
        state_adapter.message("No rival is available for relationship actions.")
        return
...
func make_alliance_offer() -> void:
    var selected := _selected_index()
    if selected < 0:
        state_adapter.message("No rival is available for an alliance offer.")
        return
...
func propose_supply_deal() -> void:
    var selected := _selected_index()
    if selected < 0:
        state_adapter.message("No rival is available for a supply deal.")
        return
...
func propose_customer_partnership() -> void:
    var selected := _selected_index()
    if selected < 0:
        state_adapter.message("No rival is available for a customer partnership.")
        return
...
func acquire_rival_asset() -> void:
    var selected := _selected_index()
    if selected < 0:
        state_adapter.message("No rival asset is currently available.")
        return
    ...
    if not bool(result.get("ok", false)):
        state_adapter.message(result.get("message", "The acquisition could not be completed.")); return
    ...
    state_adapter.message(result.get("message", "Acquisition completed."))
    state_adapter.log_message("ACQUISITION: strategic foothold purchased for $%s." % state_adapter.money(int(result.get("cost", 0))))

func negotiate_selected_acquisition() -> void: acquire_rival_asset()
func reject_selected_acquisition() -> void:
    var selected := _selected_index()
    if selected < 0:
        state_adapter.message("No rival acquisition is available to reject.")
        return
    var result = rivals.reject_acquisition(selected)
    state_adapter.message(result.get("message", "Acquisition declined.")); state_adapter.log_message("BOARDROOM: " + result.get("message", "Acquisition declined."))
