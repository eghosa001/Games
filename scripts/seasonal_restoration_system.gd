...
const PROPERTY := {"id":PROPERTY_ID,"name":"Heritage Museum House","type":"Heritage Property","description":"A historic building opened temporarily for the Heritage Restoration Festival.","value":9000,"seasonal":true}
...
func _ready() -> void:
	...
	if state!=null:
		...
		if not seasonal is Dictionary or seasonal.is_empty():
			_set_seasonal({"event_id":EVENT_ID,"active":true,"property":PROPERTY.duplicate(true),"progress":{"cleaning":0,"repair":0,"painting":0,"furnishing":0},"requirements":REQUIREMENTS.duplicate(true)})
...
func restore_step(step:String)->Dictionary:
	...
	var spend: Dictionary = state_adapter.spend(cost,"seasonal restoration: %s" % step)
...
func _complete(state,seasonal:Dictionary)->void:
	...
	_set_seasonal(seasonal)
	var history=state.get_value("company","log_lines",[]);if not history is Array:history=[];history=history.duplicate(true);history.append("HISTORY: Heritage Restoration Festival completed at day %d." % int(state.get_value("player","day",1)));if history.size()>200:history.pop_front();state.set_value("company","log_lines",history)
