extends Node
## Central presentation guard. Primary screens are mutually exclusive; screen scripts remain responsible for their own content and gameplay integration.
...
func _ui_root()->Node:
    var game_root:=get_tree().root.get_node_or_null("Renew");return game_root.get_node_or_null("UI") if game_root!=null else null
...
func show_screen(screen_name:String)->void:
    ...
    for node in _screen_nodes():var should_show:=node==target;_set_node_visible(node,should_show);_previous_visible[node.name]=should_show
