...
func _refresh() -> void:
    ...
    if items.is_empty():
        empty_label = Label.new(); empty_label.text = "NO RECOVERED ASSETS\n\nThis archive category is empty. Continue the restoration loop to unlock new historical assets, machinery, businesses and strategic rewards."; empty_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER; empty_label.vertical_alignment = VERTICAL_ALIGNMENT_CENTER; empty_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART; empty_label.add_theme_font_size_override("font_size", 13); empty_label.add_theme_color_override("font_color", MUTED); empty_label.custom_minimum_size.y = 180; content.add_child(empty_label); return
    for item in items: content.add_child(_make_item_card(item))

func _make_item_card(item: Dictionary) -> Control:
    var card := PanelContainer.new(); card.custom_minimum_size = Vector2(0, 104); card.add_theme_stylebox_override("panel", _style(SURFACE_2, BORDER, 10))
    var row := HBoxContainer.new(); row.add_theme_constant_override("separation", 12); card.add_child(row)
    var accent := ColorRect.new(); accent.custom_minimum_size = Vector2(4, 76); accent.color = ACCENT; row.add_child(accent)
    ...
    var head := Label.new(); head.text = str(item.get("title", "Recovered Asset")); head.add_theme_font_size_override("font_size", 15); head.add_theme_color_override("font_color", TEXT); head.text_overrun_behavior = TextServer.OVERRUN_TRIM_ELLIPSIS; box.add_child(head)
    var meta := Label.new(); meta.text = "%s    •    RECOVERED DAY %d" % [str(item.get("type", "")).replace("_", " ").to_upper(), int(item.get("day", 1))]; meta.add_theme_font_size_override("font_size", 9); meta.add_theme_color_override("font_color", MUTED); box.add_child(meta)
    var details: Dictionary = item.get("details", {}); var reward: Dictionary = item.get("reward", {}); var detail := Label.new(); detail.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART; detail.text = "%s\nREWARD  %s" % [_dict_text(details), _dict_text(reward)]; detail.add_theme_font_size_override("font_size", 10); detail.add_theme_color_override("font_color", MUTED); box.add_child(detail)
    return card
...
func _money(value: int) -> String: return _grouped(value)
func _grouped(value: int) -> String:
    var negative := value < 0; var digits := str(absi(value)); var out := ""
