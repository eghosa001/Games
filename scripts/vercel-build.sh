...
if [ ! -x "${GODOT_BIN}" ]; then
  curl -fsSL "https://github.com/godotengine/godot-builds/releases/download/${GODOT_VERSION}/Godot_v4.5-stable_linux.x86_64.zip" -o /tmp/godot.zip
  unzip -q -o /tmp/godot.zip -d "${GODOT_ROOT}"
  chmod +x "${GODOT_BIN}"
fi

# Godot's export template archive is published with the official builds.
...
# startup. Keep the runtime singleton names unchanged and remove only the
# conflicting optional class declarations in this isolated Vercel workspace.
for file in \
