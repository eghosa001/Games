...
func _reputation() -> int:
    if not is_inside_tree(): return 0
    var state = get_node_or_null("/root/RenewGameState")
    if state == null or not state.has_method("get_value"): return 0
...
func _culture_effect(effect_name: String, fallback: float = 1.0) -> float:
    if not is_inside_tree(): return fallback
...
func get_executives() -> Dictionary:
    ...
    for employee in employees:
        if not employee is Dictionary or employee.get("status", "active") != "active": continue
        var seat := str(employee.get("executive_seat", ""))
        if seat != "" and EXECUTIVE_SEATS.has(seat): seats[seat] = employee.duplicate(true)
    return seats

func executive_bonus_multiplier(kind: String) -> float:
    var seats := get_executives()
    match kind:
        "production": return float(EXECUTIVE_BONUSES["COO"].get("production", 1.0)) if seats.has("COO") else 1.0
        "operating_cost": return float(EXECUTIVE_BONUSES["CFO"].get("operating_cost", 1.0)) if seats.has("CFO") else 1.0
        "research": return float(EXECUTIVE_BONUSES["CTO"].get("research", 1.0)) if seats.has("CTO") else 1.0
        "hiring": return float(EXECUTIVE_BONUSES["CEO"].get("hiring", 1.0)) if seats.has("CEO") else 1.0
    return 1.0

func appoint_executive(employee_id: String, seat: String, day: int) -> Dictionary:
    seat = seat.to_upper()
    if not EXECUTIVE_SEATS.has(seat): return {"ok": false, "message": "Unknown executive seat."}
    ...
    if int(employee.get("level", 1)) < 4: return {"ok": false, "message": "%s needs career level 4 before joining the C-suite." % employee.get("name", "Employee")}
...
func remove_executive_seat(employee_id: String, day: int) -> Dictionary:
    ...
    var seat := str(employee.get("executive_seat", "")); employee["executive_seat"] = ""
    if not seat.is_empty(): _record(employee_id, "executive_unseated", {"day": day, "seat": seat})
...
func get_candidates()->Array[Dictionary]:
    ...
    for candidate in candidates:
        if candidate is Dictionary: result.append(candidate.duplicate(true))
    return result
...
func fire_employee(employee_id:String,day:int)->Dictionary:
    ...
    var dismissed_name:=str(employee.get("name","Employee"))
    var dismissed_assignment:=str(employee.get("assignment",""))
    employee["status"]="fired"; employee["morale"]=max(0,int(employee["morale"])-20); employee["loyalty"]=max(0,int(employee["loyalty"])-30)
    if str(employee.get("executive_seat",""))!="":
        var seat:=str(employee.get("executive_seat","")); employee["executive_seat"]=""; _record(employee_id,"executive_unseated",{"day":day,"seat":seat})
    var affected:=0
    for coworker in employees:
        if coworker.get("status","active")!="active" or coworker.get("id","")==employee_id: continue
        if dismissed_assignment!="" and str(coworker.get("assignment",""))!=dismissed_assignment: continue
        coworker["morale"]=max(0,int(coworker.get("morale",70))-4)
        coworker["loyalty"]=max(0,int(coworker.get("loyalty",50))-1)
        affected+=1
...
func restore_state(snapshot:Dictionary)->void:
    ...
    next_id=int(snapshot.get("next_id",employees.size()+1)); history.clear()
...
func sync_roster_to_state()->void:
    var state=get_node_or_null("/root/RenewGameState")
    if state: state.set_value("employees","roster",get_roster())
