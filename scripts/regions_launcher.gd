...
## Persistent entry points for World Command, Business Operations, Production, Supply Chain, Empire Expansion, Intelligence, Progression and Identity.
...
func _build() -> void:
    if hud == null: return
    button=_make_button("REGIONS"); operations_button=_make_button("OPS"); production_button=_make_button("PROD"); supply_button=_make_button("SUPPLY"); expansion_button=_make_button("EMPIRE"); intelligence_button=_make_button("INTEL"); progression_button=_make_button("PROG"); identity_button=_make_button("IDENT")
...
func _layout()->void:
    if button==null or operations_button==null or production_button==null or supply_button==null or expansion_button==null or intelligence_button==null or progression_button==null or identity_button==null:return
    var size:=get_viewport().get_visible_rect().size; var y:=64.0 if size.x<700.0 else 66.0; var narrow:=size.x<620.0
    var buttons:Array[Button]=[button,operations_button,production_button,supply_button,expansion_button,intelligence_button,progression_button,identity_button]
    var full:Array[String]=["REGIONS","OPS","PROD","SUPPLY","EMPIRE","INTEL","PROG","IDENT"]
    var compact:Array[String]=["REG","OPS","PRD","SUP","EMP","INT","PROG","ID"]
    if narrow:
        var gap:=3.0; var width:=maxf(34.0,(size.x-16.0-gap*7.0)/8.0); var x:=8.0
        for i in range(buttons.size()):
            buttons[i].text=compact[i]; buttons[i].size=Vector2(width,40); buttons[i].position=Vector2(x,y); buttons[i].add_theme_font_size_override("font_size",7); x+=width+gap
    else:
        var gap:=4.0; var total:=float(70+60+68+74+78+68+62+66)+gap*7.0; var start:=maxf(8.0,size.x-total-8.0); var widths:Array[float]=[70,60,68,74,78,68,62,66]; var x:=start
        for i in range(buttons.size()):
