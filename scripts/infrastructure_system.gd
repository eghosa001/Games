...
func process_day(day: int) -> Dictionary:
    ...
    for id in assets.keys():
        ...
        if str(asset.get("status")) == ACTIVE:
            var region_factor: Variant = 1.0 + float(int(asset.get("region",0)) % 3) * 0.04
            var utilization: Variant = clamp((0.45 + float(asset.get("level",1)) * 0.05) * region_factor,0.10,0.98)
...
func regional_modifier(region: int) -> Dictionary:
    var result: Variant = {"logistics":1.0,"production":1.0,"energy":1.0,"storage":1.0,"technology":1.0,"capacity":total_capacity(region)}
    for asset in assets.values():
        if int(asset.get("region",-1)) != region or str(asset.get("status")) != ACTIVE: continue
        var u: Variant = float(asset.get("utilization",0.0)); var level := float(asset.get("level",1))
...
func _register_ownership(asset_id: String, owner_id: String) -> void:
    ...
    var entity_id: Variant = "infrastructure:%s" % asset_id
