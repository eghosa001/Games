...
# RENEW V1 persistence bridge. Keeps newer empire systems in the same local save
# family as the original prototype save without forcing every system into main.gd.
...
func _state() -> Dictionary:
    var state := {"version": 1, "day": int(parent.day)}
    ...
    var corporate = parent.get_node_or_null("Corporate")
    if corporate != null:
        state["corporate"] = _corporate_state(corporate)
    return state

func _corporate_state(corporate) -> Dictionary:
    var result := {}
    for property in ["founder_ownership", "outside_investor_pool", "share_price", "company_valuation", "board_trust", "takeover_risk", "defense_level", "strategic_ally_defense", "dividend_per_share"]:
        if property in corporate:
            result[property] = corporate.get(property)
    return result

func _save_extended_state() -> void:
    if parent == null:
        return
    ...
    parent._log("SAVE: empire, supply chain, branches and regional state synchronized.")
...
func _load_extended_state() -> void:
    ...
    var supply = parent.get_node_or_null("SupplyChainController")
    if supply != null and supply_state is Dictionary:
        supply.supply.load_snapshot(supply_state)
    var contract_state = state.get("contracts", {})
    if supply != null and contract_state is Dictionary:
