...
func _layout() -> void:
    ...
    title_label.size = Vector2(w - 108.0, 30)
    ...
    status_label.position = Vector2(14, 60)
    status_label.size = Vector2(w - 28.0, 18)
    status_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_LEFT
    status_label.add_theme_font_size_override("font_size", 9 if phone else 10)
    summary_label.position = Vector2(14, 82)
    summary_label.size = Vector2(w - 28.0, 50)
