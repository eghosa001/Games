...
func get_technologies() -> Array:
    ...
    for id in TECHNOLOGIES.keys():
        var tech:Dictionary = TECHNOLOGIES[id].duplicate(true)
        tech["id"] = id
        result.append(tech)
    return result

func get_technology(id:String) -> Dictionary:
    var tech:Dictionary = TECHNOLOGIES.get(id,{})
...
func can_research(id:String) -> Dictionary:
    ...
    for prerequisite in tech.get("prerequisites",[]):
        if not is_unlocked(str(prerequisite)):
            return {"ok":false,"reason":"prerequisite","technology":str(prerequisite)}
    var state=_state()
    if state == null: return {"ok":false,"reason":"state_unavailable"}
    var cash:=int(state.get_value("economy","cash",25000))
    var points:=int(state.get_value("technology","research_points",20))
...
func research(id:String) -> bool:
    ...
    var cash:=int(state.get_value("economy","cash",25000))
    var points:=int(state.get_value("technology","research_points",20))
    state.set_value("economy","cash",cash-int(tech["cost_money"]))
    state.set_value("technology","research_points",points-int(tech["cost_points"]))
    var unlocked:Dictionary=state.get_value("technology","technology",{})
    unlocked=unlocked.duplicate(true); unlocked[id]=true
    state.set_value("technology","technology",unlocked)
    var day:=int(state.get_value("player","day",1))+int(tech["time_days"])
    state.set_value("player","day",day)
    state.set_value("company","message","Research complete: %s. %d day(s) passed." % [tech["name"],int(tech["time_days"])])
    state.log_message("TECHNOLOGY: %s researched (-$%s, -%d RP, %d days)." % [tech["name"],state.money(int(tech["cost_money"])),int(tech["cost_points"]),int(tech["time_days"])])
    return true
    ...
    for tier in [1,2,3]:
        for id in TECHNOLOGIES.keys():
            if int(TECHNOLOGIES[id]["tier"]) == tier and not is_unlocked(id) and bool(can_research(id).get("ok",false)):
                return research(id)
    var state=_state()
    if state != null: state.set_value("company","message","No technology is currently researchable.")
    return false

func add_daily_research_points(amount:int=3) -> void:
    var state=_state()
    if state == null: return
    var points:=int(state.get_value("technology","research_points",20))+max(0,amount)
    state.set_value("technology","research_points",points)
...
func effect(name:String,default_value:float=0.0) -> float:
    ...
    var unlocked=state.get_value("technology","technology",{})
    if not unlocked is Dictionary: return total
    for id in unlocked.keys():
        if not bool(unlocked[id]): continue
        var effects:Dictionary=TECHNOLOGIES.get(str(id),{}).get("effects",{})
        total += float(effects.get(name,0.0))
    return total

func production_multiplier() -> float: return max(1.0,1.0+effect("production_multiplier",0.0))
func worker_reduction() -> int: return max(0,int(effect("worker_reduction",0.0)))
func maintenance_multiplier() -> float: return max(0.0,1.0+effect("maintenance_multiplier",0.0))
func input_efficiency() -> float: return max(0.0,min(0.50,effect("input_efficiency",0.0)))
func transport_capacity_multiplier() -> float: return max(1.0,1.0+effect("transport_capacity_multiplier",0.0))
func operating_cost_multiplier() -> float: return max(0.25,1.0+effect("operating_cost_multiplier",0.0))
func base_price_multiplier() -> float: return max(1.0,1.0+effect("base_price_multiplier",0.0))
func market_demand_multiplier() -> float: return max(1.0,1.0+effect("market_demand_multiplier",0.0))
