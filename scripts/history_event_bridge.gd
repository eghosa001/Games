...
func _check_state() -> void:
    ...
    _contract_events(history,state,day); _acquisition_events(history,state,day); _technology_events(history,state,day); _alliance_events(history,state,day); _project_events(history,state,day); _competitor_events(history,state,day); _major_events(history,state,day); _ingest_log(history,state,day)

func _exit_tree() -> void:
    if check_timer != null and is_instance_valid(check_timer):
        if check_timer.timeout.is_connected(_check_state): check_timer.timeout.disconnect(_check_state)
        check_timer.stop()
...
func _contract_events(history,state,day:int)->void:
    ...
    var signed=int(contracts.get("signed_count",contracts.get("active_count",0))); var fulfilled=int(contracts.get("fulfilled_count",contracts.get("completed_count",0)))
    _counter(history,"contracts_signed",signed,day,"CONTRACT_SIGNED","Contract signed",{"count":signed}); _counter(history,"contracts_fulfilled",fulfilled,day,"CONTRACT_FULFILLED","Contract fulfilled",{"count":fulfilled})
func _acquisition_events(history,state,day:int)->void:
    var count:=int(state.get_value("ownership","acquisition_count",0)); _counter(history,"acquisitions_completed",count,day,"ACQUISITION","Acquisition completed",{"count":count})
...
func _alliance_events(history,state,day:int)->void:
    ...
    var count: int =alliances.size(); _counter(history,"alliances_formed",count,day,"ALLIANCE_FORMED","Alliance formed",{"count":count})
    var completed:=0
...
func _project_events(history,state,day:int)->void:
    var reputation=int(state.get_value("regions","regional_reputation",0)); _counter(history,"regional_reputation",reputation,day,"PROJECT_COMPLETED","Regional cooperative project completed",{"regional_reputation":reputation})
