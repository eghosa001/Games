...
func found_new_company()->Dictionary:
    var victory=get_node_or_null("/root/RenewVictorySystem")
    if victory==null or not victory.has_method("found_new_company"):
        _set_state("company","message","New dynasties are unavailable.")
        return {"ok":false,"message":"New dynasties are unavailable."}
...
func _restore_daily_transaction(snapshot:Dictionary)->void:
    ...
    for participant in _daily_transaction_participants():
        ...
        if node==null or not state is Dictionary:continue
        if name=="game_state" and node.has_method("restore"):node.restore(state)
        elif name=="ownership" and not node.has_method("restore_state") and node.has_method("load_state"):node.load_state(state)
        elif node.has_method("restore_state"):node.restore_state(state)
...
func advance_day()->void:
    ...
    if units_sold>0:
        ...
        if not (sold_product in ["furniture","construction_materials","consumer_electronics"]):sold_product="furniture"
