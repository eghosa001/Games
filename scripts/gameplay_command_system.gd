...
# Phase 3 command bus. Gameplay rules live in domain systems. This class only
# routes commands and orchestrates the one cross-domain simulation operation.
...
var employee_system = EmployeeCommandSystem.new()
var finance_system = FinanceCommandSystem.new()
var supply_system = SupplyCommandSystem.new()
var contract_system = ContractCommandSystem.new()
...
func _log(text: String) -> void:
    ...
    _set_state("company", "log_lines", logs)
...
func load_game() -> void:
    var snapshot = SaveSystem.load_game()
