...
func acquire_property() -> void:
    ...
    if owned: message = "You already own it."; return
    if cash < property_cost: message = "Not enough cash."; return
    cash -= property_cost; owned = true; reputation += 2
...
func restore_property() -> void:
    ...
    if cash < cost: message = "Need $%s for the next restoration stage." % _money(cost); return
...
func open_business() -> void:
    ...
    cash -= 3000; business_open = true; finished_goods = 0; reputation += 2
...
func buy_inputs() -> void:
    ...
    if randi_range(1, 100) > int(supplier["reliability"]):
        ...
        _log("SUPPLIER FAILURE: %s delayed your inputs." % supplier["name"]); return
    var total_cost := 0
    for resource in ["materials", "packaging", "fuel"]:
        total_cost += int(economy.resources[resource]["price"]) * 12
    if cash < total_cost:
        message = "You need $%s for the complete input order." % _money(total_cost); return
    for resource in ["materials", "packaging", "fuel"]:
        economy.resources[resource]["stock"] += 12
    cash -= total_cost
...
func produce_goods() -> void:
    ...
    if not result["ok"]: message = "Production stopped: not enough materials, packaging or fuel."; return
...
func hire_employee() -> void:
    ...
    message = "Hired employee %d. Daily wages increased." % employees
    _log("HIRING: employee %d joined the company (-$%s)." % [employees, _money(cost)])
...
func upgrade_business() -> void:
    ...
    _log("UPGRADE: production capacity level %d (-$%s)." % [capacity_level, _money(cost)])
...
func marketing_campaign() -> void:
    ...
    message = "Marketing level %d. Customers will value your brand more." % marketing_level
    _log("MARKETING: campaign launched (-$%s)." % _money(cost))
...
func advance_day() -> void:
    ...
    if finished_goods < 5: message = "You have no finished goods. Buy inputs and produce first."; return
    var rival_price: int = int(rivals.rivals[0]["price"])
    var price_factor := clamp(float(rival_price - player_price) / 50.0, -0.55, 0.75)
    var demand := clamp(int(55.0 * (0.7 + price_factor) + reputation * 0.7 + marketing_level * 8), 5, 100)
    ...
    cash += profit; finished_goods -= units; last_sales = sales; last_profit = profit; total_profit += profit
...
func select_rival(index: int) -> void:
    ...
    selected_rival = index; relationship = int(rivals.rivals[selected_rival]["relationship"])
...
func select_expansion(index: int) -> void:
    ...
    selected_expansion = index; message = "Selected %s." % expansion.properties[selected_expansion]["name"]
...
func save_game() -> void:
    var state := {"cash":cash,"reputation":reputation,"day":day,"owned":owned,"inspected":inspected,"restoration":restoration,"stage":stage,"business_open":business_open,"employees":employees,"capacity_level":capacity_level,"marketing_level":marketing_level,"finished_goods":finished_goods,"player_price":player_price,"total_profit":total_profit}
...
func load_game() -> void:
    ...
    message = "Game loaded."; _log("SAVE: Previous company state restored.")
...
func _draw() -> void:
    ...
    draw_string(ThemeDB.fallback_font,Vector2(350,370),"[I] Inspect [A] Acquire [R] Restore [O] Open",HORIZONTAL_ALIGNMENT_LEFT,-1,14,Color("c6d0d8"))
    ...
    if business_open:
        draw_string(ThemeDB.fallback_font,Vector2(664,190),"Employees %d | Capacity Lv.%d | Marketing Lv.%d"%[employees,capacity_level,marketing_level],HORIZONTAL_ALIGNMENT_LEFT,-1,16,Color("d0dae1"))
        draw_string(ThemeDB.fallback_font,Vector2(664,216),"Finished goods %d | Price $%s"%[finished_goods,_money(player_price)],HORIZONTAL_ALIGNMENT_LEFT,-1,16,Color("d0dae1"))
        draw_string(ThemeDB.fallback_font,Vector2(664,242),"Last sales $%s | Profit $%s"%[_money(last_sales),_money(last_profit)],HORIZONTAL_ALIGNMENT_LEFT,-1,16,Color("8ee6a8"))
        draw_string(ThemeDB.fallback_font,Vector2(664,268),"Inputs: M $%d | P $%d | F $%d"%[int(economy.resources["materials"]["price"]),int(economy.resources["packaging"]["price"]),int(economy.resources["fuel"]["price"])],HORIZONTAL_ALIGNMENT_LEFT,-1,16,Color("c6d0d8"))
        var supplier: Dictionary = economy.supplier_for(supplier_resource)
        draw_string(ThemeDB.fallback_font,Vector2(664,294),"Supplier focus: %s | reliability %d%%"%[supplier["name"],supplier["reliability"]],HORIZONTAL_ALIGNMENT_LEFT,-1,15,Color("f2d27a"))
        draw_string(ThemeDB.fallback_font,Vector2(664,326),"[S] Supplies [B] Produce [H] Hire [U] Upgrade",HORIZONTAL_ALIGNMENT_LEFT,-1,14,Color.WHITE)
        draw_string(ThemeDB.fallback_font,Vector2(664,351),"[M] Marketing [P] Price [N] End day [T] Supplier",HORIZONTAL_ALIGNMENT_LEFT,-1,14,Color("b7d7ff"))
        draw_string(ThemeDB.fallback_font,Vector2(664,376),"Rival %s | relationship %d | [1-3] select"%[rivals.rivals[selected_rival]["name"],relationship],HORIZONTAL_ALIGNMENT_LEFT,-1,14,Color("d0dae1"))
        draw_string(ThemeDB.fallback_font,Vector2(664,401),"[L] Trust [C] Alliance [E] Expansion [F5/F9] Save/Load",HORIZONTAL_ALIGNMENT_LEFT,-1,14,Color("b7d7ff"))
    else:
    ...
    draw_string(ThemeDB.fallback_font,Vector2(52,530),"Giant / Specialist / Network",HORIZONTAL_ALIGNMENT_LEFT,-1,17,Color.WHITE)
    draw_string(ThemeDB.fallback_font,Vector2(52,557),"[1-3] rival | [L] trust | [C] alliance",HORIZONTAL_ALIGNMENT_LEFT,-1,15,Color("c6d0d8"))
    expansion.unlock_from_reputation(reputation)
    draw_string(ThemeDB.fallback_font,Vector2(52,584),"Expansion %d/%d | [7-9] select | %s"%[expansion.unlocked,expansion.properties.size(),expansion.properties[selected_expansion]["name"]],HORIZONTAL_ALIGNMENT_LEFT,-1,15,Color("f2d27a"))
    draw_string(ThemeDB.fallback_font,Vector2(52,610),"Goal: restore the world, build businesses, make allies and compete for control.",HORIZONTAL_ALIGNMENT_LEFT,-1,15,Color("8ee6a8"))
    ...
    draw_string(ThemeDB.fallback_font,Vector2(52,655),message,HORIZONTAL_ALIGNMENT_LEFT,1160,15,Color("f2d27a"))
    draw_string(ThemeDB.fallback_font,Vector2(52,682),"Core loop: RESTORE → PRODUCE → SELL → REINVEST → COMPETE → ALLY → EXPAND",HORIZONTAL_ALIGNMENT_LEFT,-1,14,Color("b7d7ff"))
...
func _draw_warehouse(o: Vector2) -> void:
    ...
    if restoration < 40:
        draw_circle(o+Vector2(32,155),8,Color("675c4d")); draw_circle(o+Vector2(205,158),10,Color("675c4d"))
    if restoration >= 100:
