...
var log_lines:Array:
    get:var value=_state_value("company","log_lines",[]);return value.duplicate(true) if value is Array else []
...
func _state_value(domain:String,key:String,default_value):var state=_game_state();return default_value if state==null else state.get_value(domain,key,default_value)
func _set_state_value(domain:String,key:String,value)->void:var state=_game_state();if state!=null:state.set_value(domain,key,value)
func _ready()->void:add_child(command_system);command_system.initialize();add_child(customer_segments_ui);refresh_ui()
func _process(_delta:float)->void:refresh_ui()
func refresh_ui()->void:queue_redraw()
func _dispatch(command:String,args:Array=[])->void:if command_system!=null:command_system.callv(command,args);refresh_ui()
func _input(event:InputEvent)->void:
    if event is not InputEventKey or not event.pressed or event.echo:return
    if event.shift_pressed:
        match event.keycode:
            KEY_1:_dispatch("research_technology",["efficient_production"])
            KEY_2:_dispatch("research_technology",["better_logistics"])
            KEY_3:_dispatch("research_technology",["market_analysis"])
            KEY_4:_dispatch("research_technology",["automation"])
            KEY_5:_dispatch("research_technology",["advanced_materials"])
            KEY_6:_dispatch("research_technology",["supply_optimization"])
            KEY_7:_dispatch("research_technology",["smart_factory"])
            KEY_8:_dispatch("research_technology",["advanced_logistics"])
            KEY_9:_dispatch("research_technology",["premium_manufacturing"])
            KEY_0:_dispatch("research_next_technology")
        return
    match event.keycode:
        ...
        KEY_Y:_dispatch("research_next_technology")
...
func research_technology(id:String)->void:_dispatch("research_technology",[id])
func research_next_technology()->void:_dispatch("research_next_technology")
