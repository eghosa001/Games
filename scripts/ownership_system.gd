...
func register_entity(entity_id: String, entity_type: String = ENTITY_COMPANY, total_shares: int = 1000000, par_value: float = 1.0) -> Dictionary:
    ...
    entities[entity_id] = {
        ...
        "holders": {},
        "share_classes": {
            VOTE_ORDINARY: {"votes_per_share": 1.0, "dividend_multiplier": 1.0},
            VOTE_NON_VOTING: {"votes_per_share": 0.0, "dividend_multiplier": 1.0}
        },
        "control_thresholds": {"ordinary": 0.50},
        "board": {"seat_count": 3, "seats": {}},
        "investor_confidence": 50.0,
        "defense_level": 0,
        "poison_pill": false,
        "dividend_pool": 0.0
    }
    _record(entity_id, "register", {"type": entity_type, "authorized_shares": total_shares, "par_value": par_value})
...
func issue_shares(entity_id: String, holder_id: String, quantity: int, share_class: String = VOTE_ORDINARY, reason: String = "issue") -> Dictionary:
    if not entities.has(entity_id) or holder_id.is_empty() or quantity <= 0:
        return {"ok": false, "error": "invalid_issuance"}
    var entity: Dictionary = entities[entity_id]
    if not entity["share_classes"].has(share_class):
        return {"ok": false, "error": "unknown_share_class"}
    if int(entity["issued_shares"]) + quantity > int(entity["authorized_shares"]):
        return {"ok": false, "error": "authorized_share_limit"}
    var holder: Dictionary = _holder(entity, holder_id)
...
func issue_for_percent(entity_id: String, holder_id: String, percent_of_post_issue: float, share_class: String = VOTE_ORDINARY, reason: String = "issue_for_percent") -> Dictionary:
    if not entities.has(entity_id) or percent_of_post_issue <= 0.0 or percent_of_post_issue >= 100.0:
        return {"ok": false, "error": "invalid_target_percent"}
    var issued: float = float(entities[entity_id]["issued_shares"])
    var quantity: Variant = int(ceil(issued * percent_of_post_issue / (100.0 - percent_of_post_issue)))
    if quantity <= 0:
        quantity = 1
...
func transfer_shares(entity_id: String, from_holder: String, to_holder: String, quantity: int, share_class: String = VOTE_ORDINARY, reason: String = "transfer") -> Dictionary:
    ...
    var entity: Dictionary = entities[entity_id]
    var source: Dictionary = _holder(entity, from_holder)
    if _shares(source, share_class) < quantity:
        return {"ok": false, "error": "insufficient_shares"}
    var destination: Dictionary = _holder(entity, to_holder)
...
func buyback(entity_id: String, holder_id: String, quantity: int, price_per_share: float = 0.0, reason: String = "buyback") -> Dictionary:
    ...
    entities[entity_id] = entity
    var cost: float = float(quantity) * max(0.0, price_per_share)
...
func get_voting_power(entity_id: String, holder_id: String) -> float:
    var entity: Dictionary = _entity(entity_id)
    if entity.is_empty(): return 0.0
    var holder: Dictionary = entity["holders"].get(holder_id, {})
    var votes: float = 0.0
...
func get_ownership_percent(entity_id: String, holder_id: String) -> float:
    ...
    var holder: Dictionary = entity.get("holders", {}).get(holder_id, {})
    var shares: int = 0
    for share_class_name in holder.get("classes", {}):
...
func get_voting_percent(entity_id: String, holder_id: String) -> float:
    var total: float = get_total_votes(entity_id)
...
func has_control(entity_id: String, holder_id: String, threshold_name: String = "ordinary") -> bool:
    var entity: Dictionary = _entity(entity_id)
    var threshold: float = float(entity.get("control_thresholds", {}).get(threshold_name, 0.50)) * 100.0
...
func appoint_board_seat(entity_id: String, holder_id: String, seat_id: String = "") -> Dictionary:
    ...
    var seat_count: int = int(board["seat_count"])
...
func board_control_percent(entity_id: String, holder_id: String) -> float:
    var entity: Dictionary = _entity(entity_id)
    var seats: int = int(entity.get("board", {}).get("seat_count", 0))
...
func distribute_dividend(entity_id: String, amount: float, holder_filter: String = "") -> Dictionary:
    ...
    var entity: Dictionary = entities[entity_id]
    var issued: float = float(entity["issued_shares"])
    if issued <= 0.0:
    ...
    for holder_id in entity["holders"]:
        if not holder_filter.is_empty() and str(holder_id) != holder_filter:
            continue
        var holder: Dictionary = entity["holders"][holder_id]
        var weight: float = 0.0
        for share_class_name in holder.get("classes", {}):
            var multiplier: float = float(entity["share_classes"].get(share_class_name, {}).get("dividend_multiplier", 1.0))
            weight += float(holder["classes"][share_class_name]) * multiplier
        weights[holder_id] = weight
        eligible_weight += weight
    if eligible_weight <= 0.0:
        return {"ok": false, "error": "no_dividend_eligible_shares"}
    var payouts: Dictionary = {}
    var total_paid: float = 0.0
    for holder_id in weights:
        var payout: float = amount * float(weights[holder_id]) / eligible_weight
...
func adjust_investor_confidence(entity_id: String, delta: float, reason: String = "") -> Dictionary:
    if not entities.has(entity_id): return {"ok": false, "error": "entity_not_found"}
    var confidence: float = clamp(float(entities[entity_id]["investor_confidence"]) + delta, 0.0, 100.0)
...
func takeover_control(entity_id: String, bidder_id: String, required_percent: float = 50.0) -> Dictionary:
    ...
    var current: float = get_voting_percent(entity_id, bidder_id)
    ...
    var entity: Dictionary = entities[entity_id]
    var board_percent: float = board_control_percent(entity_id, bidder_id)
    var board_majority: bool = board_percent > 50.0
...
func get_control_snapshot(entity_id: String) -> Dictionary:
    ...
    var top_holder: String = ""
    var top_ownership: float = 0.0
...
func _record(entity_id: String, action: String, details: Dictionary) -> void:
    transaction_log.append({"id": next_transaction_id, "entity_id": entity_id, "action": action, "details": details.duplicate(true)})
