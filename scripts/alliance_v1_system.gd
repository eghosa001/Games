...
func invite_member(alliance_id: String, inviter_id: String, member_id: String) -> Dictionary:
    var alliances := _alliances(); var alliance: Dictionary = alliances.get(alliance_id, {})
...
func leave_alliance(member_id: String = PLAYER_ID) -> Dictionary:
    var alliances := _alliances(); var id := _member_alliance(member_id, alliances)
...
func post_message(alliance_id: String, member_id: String, text: String) -> Dictionary:
    var alliances := _alliances(); var alliance: Dictionary = alliances.get(alliance_id, {}); text = text.strip_edges()
...
func donate_resource(member_id: String, resource_id: String, amount: int) -> Dictionary:
    ...
    var alliances := _alliances(); var id := _member_alliance(member_id, alliances)
    if id.is_empty(): return {"ok": false, "message": "Member is not in an alliance."}
    var resources := _resources(); if int(resources.get(resource_id, 0)) < amount: return {"ok": false, "message": "Not enough %s to donate." % resource_id}
...
func contribute_to_railway(member_id: String, money: int = 0, steel: int = 0, timber: int = 0, project_points: int = 0) -> Dictionary:
    ...
    var resources := _resources(); if steel > int(resources.get("steel", 0)) or timber > int(resources.get("timber", 0)): return {"ok": false, "message": "Not enough steel or timber for this railway contribution."}
...
func get_member_alliance(member_id: String = PLAYER_ID) -> Dictionary:
    var alliances := _alliances(); var id := _member_alliance(member_id, alliances); return alliances.get(id, {}).duplicate(true)
